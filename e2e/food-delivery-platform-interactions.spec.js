...
test('Food Platform controls and checkout produce a complete in-app order flow', async ({ page }) => {
  const pageErrors = []
  ...
  await expect(page.getByTestId('food-delivery-platform-merchant-platform_hanwoo_gukbap')).toHaveCount(0)
  ...
  await expect(page.getByTestId('food-delivery-platform-merchant-summary')).toContainText('随机推荐 3 家 · 共 11 家')
  ...
  await expect(page.getByTestId('food-delivery-platform-banner-dot-0')).toHaveAttribute('aria-current', 'true')
  await expect(bannerRail).toHaveAttribute('data-active-banner-index', '1', { timeout: 7000 })
  await expect.poll(() => bannerScroller.evaluate((element) => element.scrollLeft)).toBeGreaterThan(250)
  ...
  await expect.poll(() => bannerScroller.evaluate((element) => element.scrollLeft)).toBeGreaterThan(500)
  ...
  ).toHaveAttribute('data-required-asset', /merchant-logo-morning-bagel-01\.png/)
  await expect(
    page.getByTestId('food-delivery-platform-merchant-card-platform_golden_chicken'),
  ).toHaveAttribute('data-merchant-visual-type', 'logo')
  ...
  await expect(page.getByTestId('food-delivery-platform-campaign-merchants')).toContainText('逆站洞韩牛汤饭')
  await page.getByTestId('food-delivery-platform-campaign-primary').click()
  await expect(page.getByTestId('food-delivery-platform-benefit-feedback')).toContainText('权益已加入')
  ...
  await expect(page.getByTestId('food-delivery-platform-campaign-benefits')).toContainText('三种好运')
  await page.getByTestId('food-delivery-platform-campaign-primary').click()
  await expect(page.getByTestId('food-delivery-platform-campaign-prize')).toBeVisible()
  await expect(page.getByTestId('food-delivery-platform-campaign-prize')).toContainText(/满 49 减 8|0 元配送券|甜品加赠签/)
  await page.getByTestId('food-delivery-platform-campaign-back').click()
  ...
  await expect(page.getByTestId('food-delivery-platform-campaign-menu-picks')).toContainText('GOOD AM 烟熏鸡贝果')
  await page.getByTestId('food-delivery-platform-campaign-back').click()

  await page.getByTestId('food-delivery-platform-save-platform_hanwoo_gukbap').click()
  await expect(page.getByTestId('food-delivery-platform-save-platform_hanwoo_gukbap')).toHaveAttribute(
    'aria-pressed',
    'true',
  )
  ...
  await expect(page.getByTestId('food-delivery-platform-nav-saved')).toHaveAttribute('aria-current', 'page')
  await expect(page.getByTestId('food-delivery-platform-saved-grid')).toContainText('逆站洞韩牛汤饭')
  ...
  await expect(page.getByTestId('food-delivery-platform-merchant-menu')).toContainText('海风泡菜煎饼')
  ...
  await expect(page.getByTestId('food-delivery-platform-menu-quantity-platform_hanwoo_gukbap_menu_1')).toHaveText('1')
  ...
  await expect(page.getByTestId('food-delivery-platform-cart-line-platform_hanwoo_gukbap_menu_1')).toContainText(
    '逆站洞一号韩牛汤饭',
  )
  await expect(page.getByTestId('food-delivery-platform-cart-total')).toContainText('58.00')
  await page.getByTestId('food-delivery-platform-cart-increase-platform_hanwoo_gukbap_menu_1').click()
  await expect(page.getByTestId('food-delivery-platform-cart-quantity-platform_hanwoo_gukbap_menu_1')).toHaveText('2')
  ...
  await expect(page.locator('[data-asset-slot="platform-checkout-takeout-bag"] img')).toHaveAttribute(
    'src',
    /missing-asset-placeholder\.svg/,
  )
  ...
  await expect(page.getByTestId('food-delivery-platform-order-success')).toHaveAttribute('data-order-status', 'placed')
  await expect(page.locator('[data-asset-slot="platform-order-status-placed"]')).toBeVisible()
  await expect(page.getByTestId('food-delivery-platform-bottom-nav')).toHaveCount(0)
  await expect(page.getByTestId('food-delivery-platform-order-summary')).toContainText('逆站洞一号韩牛汤饭')
  await expect(page.getByTestId('food-delivery-platform-order-summary')).toContainText('116.00')
  const orderId = await page.getByTestId('food-delivery-platform-order-id').getAttribute('data-order-id')
  await expect(page.getByTestId('food-delivery-platform-order-id')).toHaveText(/^FD\d{6}-[A-Z0-9]{4}$/)
  await expectNoHorizontalOverflow(page)

  await page.getByTestId('food-delivery-platform-view-orders').click()
  await expect(page).toHaveURL(/platformView=orders/)
  await expect(page.getByTestId('food-delivery-platform-order-list')).toContainText('逆站洞韩牛汤饭')
  ...
  await expect(page.getByTestId('food-delivery-platform-order-id')).toHaveAttribute('data-order-id', orderId)
  ...
  await expect(page.getByTestId('food-delivery-platform-profile-services')).toContainText('配送与沟通')
