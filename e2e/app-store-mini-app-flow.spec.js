...
test('App Store mini apps hand off to Food Delivery and Shopping without deleting source records', async ({
  ...
  await expect(page.locator('h1')).toContainText('Kurly')

  await navigateInsideUnlockedApp(page, '/shopping/schat_mall')
  await expect(page.getByTestId('shopping-service-daily_fresh')).toHaveCount(0)
  await expect(page.getByTestId('shopping-service-schat_mall')).toHaveCount(0)
  await expectNoHorizontalOverflow(page)
