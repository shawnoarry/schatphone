...
### World Semantic Proposal And Compiled Manifest

Arbitrary user-authored world text requires a versioned semantic boundary before it can safely drive reusable event eligibility. The accepted direction separates authored sources, temporary model proposals, user-reviewed semantic manifests, deterministic compiled runtime manifests, and owner-confirmed runtime truth. AI may interpret custom terms and propose capabilities, boundaries, unknowns, and conflicts; it cannot directly create the reviewed manifest, event truth, owner facts, identities, permissions, random settlement, or numeric effects.

`Settings > World Setup` implements the ordinary read-only preparation and diagnostic preview in `src/lib/simulation/world-semantic-preview.js`. Opening Settings or the subpage performs no provider call. One explicit configured-model call returns a bounded provider-neutral proposal with provenance; user-facing copy states that it is only a check and has changed no setting. It is not persisted or consumed by Event Runtime. World Hub exposes only advanced world-usage evidence and does not own the check.

The formal pure boundary is implemented in `src/lib/simulation/world-semantic-contract.js` and `src/lib/simulation/world-semantic-compiler.js`. The first separates normalized proposals, provider/model receipts, exact user confirmation, and source/proposal hashes. The second validates a versioned generic runtime registry, rejects invalid/dangling/unsupported/ownerless/unresolved meaning, and produces immutable deterministic indexes plus a manifest hash without importing Stores or provider fields. World Setting W2 now persists immutable reviewed/compiled versions with activation, rollback, restore validation, and exact Event Instance binding. Modern K-pop, magic academy, and Tide Contract City use equal fixtures and the same compiler path. See `docs/pm/event-runtime-and-world-hub/WORLD_SEMANTIC_RUNTIME_FOUNDATION_PLAN_2026-08-29.md` and `docs/pm/event-runtime-and-world-hub/WORLD_SEMANTIC_CONFIRMATION_VERSIONING_ROLLBACK_CONTRACT_2026-08-29.md`.

`WORLD-SEMANTIC-4` adds the first production consumer without turning the semantic manifest into owner truth. `src/lib/simulation/world-semantic-access-runtime.js` matches Map-owned place concepts and Work Hub-owned membership/role concepts through `runtime:access:restricted_place`. Event Runtime creates one stable Event Instance V2 bound to the exact world version, map, place, and position evidence, issues one Map validation request, accepts one correlated Map owner fact, and settles one durable routine/reviewed result. Randomness cannot create qualification or reverse denial. Map creates its existing place session only after a stored grant. Missing/stale/mismatched evidence fails closed, unannotated places retain the ordinary path, reopen/restore cannot reroll, and a later semantic version applies only to new event identities. No provider call occurs during play; Settings remains the normal preparation surface and World Hub remains advanced audit.
...
9. `WORLD-SEMANTIC-4 DONE 2026-08-31`: confirmed arbitrary-world semantics now drive one generic restricted-place entry family through exact Map place evidence, valid Work Hub membership/role evidence, one Event Instance V2, one Map owner fact, durable routine/reviewed settlement, fail-closed native feedback, and reopen/backup reuse. Three different world fixtures use one engine path with no genre branch or runtime model call.
10. `EVE-5`: use the shared Mini Scene Interface for richer expansion only after its persistence, Settings, and Presenter gates are complete; later CG remains a separate image-generation/media-resolution stage.
...
- `world-semantic-access-runtime.js`, `world-semantic-access-event-templates.js`, and `world-semantic-access-event-runtime.js` implement the generic restricted-place resolver, built-in Event Instance V2 family, stable occurrence identity, and Map owner-fact contract; `store:map` orchestrates the exact owner actions while preserving ordinary unannotated entry
...
- preserve the completed `WORLD-SEMANTIC-4` proof: do not rerun old occurrences after world updates, infer concepts from genre/prose, call a model during entry, let Runtime or Work Hub write Map truth, expose World Hub as a required user path, or broaden one restricted-place capability into universal world simulation without a new roadmap gate
