...
The formal pure boundary is now implemented in `src/lib/simulation/world-semantic-contract.js` and `src/lib/simulation/world-semantic-compiler.js`. The first separates normalized proposals, provider/model receipts, exact user confirmation, and source/proposal hashes. The second validates a versioned generic runtime registry, rejects invalid/dangling/unsupported/ownerless/unresolved meaning, and produces immutable deterministic indexes plus a manifest hash without importing Stores or provider fields. Modern K-pop, magic academy, and Tide Contract City use equal fixtures and the same compiler path. Persisted compilation and activation still wait for World Setting W2 identity and atomic rollback. See `docs/pm/event-runtime-and-world-hub/WORLD_SEMANTIC_RUNTIME_FOUNDATION_PLAN_2026-08-29.md` and `docs/pm/event-runtime-and-world-hub/WORLD_SEMANTIC_CONFIRMATION_VERSIONING_ROLLBACK_CONTRACT_2026-08-29.md`.
