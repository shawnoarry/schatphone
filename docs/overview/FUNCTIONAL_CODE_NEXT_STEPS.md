...
20. Next recommended slice: continue low-risk placeholder hardening, such as wiring More toggles to real persisted flags, or add automated component-level coverage for the Network smoke controls.
    下一推荐切片：继续低风险占位模块实装，例如将 More 开关接入真实持久化标志，或为 Network 烟测控件补组件级自动化覆盖。
21. Keep deferring visual redesign and broad store rewrites.
