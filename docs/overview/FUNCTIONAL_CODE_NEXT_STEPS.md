...
13. Next recommended slice: switch to Network guided setup, or extract a shared thumbnail picker if continuing decomposition.
    下一推荐切片：切换到 Network 引导配置；若继续拆分，则抽共享缩略图选择器。
14. Keep deferring visual redesign and broad store rewrites.
...
1. Network guided setup and connection testing polish.
   Network 引导式配置与连接测试打磨。
...
Start Network guided setup if the goal shifts from decomposition to making provider setup easier for non-technical users.
The shared thumbnail cleanup now covers the major asset-consuming surfaces; move to Network guided setup unless a review finds a regression.

如果目标从拆分转为降低非技术用户配置供应商的难度，优先做 Network 引导配置。
共享缩略图清理已覆盖主要素材消费界面；除非 review 发现回归，否则下一步转向 Network 引导配置。
