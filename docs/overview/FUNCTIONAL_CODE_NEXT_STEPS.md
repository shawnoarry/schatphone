...
21. Next recommended slice: continue low-risk placeholder hardening, such as adding Profile AI-context visibility, or add automated component-level coverage for the Network smoke controls.
    下一推荐切片：继续低风险占位模块实装，例如补 Profile 的 AI 上下文可见性，或为 Network 烟测控件补组件级自动化覆盖。
22. Keep deferring visual redesign and broad store rewrites.
