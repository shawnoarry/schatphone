...
17. Next recommended slice: continue Network preset safety, especially save-time quality warnings and clearer key-handling copy.
    下一推荐切片：继续提升 Network 预设安全性，优先补保存预设时的质量警告与更清晰的 Key 处理文案。
18. Keep deferring visual redesign and broad store rewrites.
...
1. Network preset safety after endpoint/gateway guidance.
   Network 接口/网关指引后的预设安全性提升。
...
Start Network preset safety if the goal remains making provider setup easier for non-technical users.
Network guided setup, connection-test failure guidance, and endpoint/gateway checks now cover the first painful setup loop; the next practical slice is save-time quality warnings and clearer key-handling copy.

如果目标仍是降低非技术用户配置供应商的难度，优先做 Network 预设安全性提升。
Network 引导配置、连接测试失败指引与接口/网关检查已覆盖第一轮痛点；下一刀适合补保存预设时的质量警告与更清晰的 Key 处理文案。
