...
16. Next recommended slice: continue Network provider-preset quality, especially safer custom gateway guidance and manual-model fallback confirmation.
    下一推荐切片：继续提升 Network 供应商预设质量，优先补自定义网关安全指引与手动模型兜底确认。
17. Keep deferring visual redesign and broad store rewrites.
...
1. Network provider-preset quality after guided setup and failure guidance.
   Network 引导配置与失败指引后的供应商预设质量提升。
2. Continue small Network UX hardening before returning to broad feature work.
   回到大功能前，继续做小体量 Network 体验加固。

My recommendation / 我的建议:

Start Network provider-preset quality if the goal remains making provider setup easier for non-technical users.
Network guided setup and connection-test failure guidance now cover the first painful setup loop; the next practical slice is safer custom gateway guidance and manual-model fallback confirmation.

如果目标仍是降低非技术用户配置供应商的难度，优先做 Network 供应商预设质量提升。
Network 引导配置与连接测试失败指引已覆盖第一轮痛点；下一刀适合补更安全的自定义网关提示与手动模型兜底确认。
