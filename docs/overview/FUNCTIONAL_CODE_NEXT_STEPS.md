...
15. Next recommended slice: continue Network connection-testing polish, especially clearer failure guidance and diagnostics handoff.
    下一推荐切片：继续 Network 连接测试打磨，优先补清晰失败指引与诊断交接。
16. Keep deferring visual redesign and broad store rewrites.
...
1. Network connection testing polish after guided setup phase 1.
   Network 引导配置第一阶段后的连接测试打磨。
2. Commit the current maintainability batch, then switch to Network guided setup.
   提交当前可维护性批次，然后切换到 Network 引导配置。

My recommendation / 我的建议:

Start Network connection-testing polish if the goal shifts from decomposition to making provider setup easier for non-technical users.
Network guided setup phase 1 now covers provider templates, setup progress, and next-step copy; the next practical slice is clearer failure guidance and diagnostics handoff after a test attempt.

如果目标从拆分转为降低非技术用户配置供应商的难度，优先做 Network 连接测试打磨。
Network 引导配置第一阶段已覆盖供应商模板、配置进度与下一步提示；下一刀适合补测试失败后的清晰指引与诊断交接。
