...
12. Next recommended slice: extract `MapTripHistoryPanel`, or switch to Network guided setup.
    下一推荐切片：抽 `MapTripHistoryPanel`，或切换到 Network 引导配置。
13. Keep deferring visual redesign and broad store rewrites.
...
1. Map derived display extraction: extract `MapTripHistoryPanel`.
   Map 派生展示拆分：抽 `MapTripHistoryPanel`。
2. Network guided setup and connection testing polish.
   Network 引导式配置与连接测试打磨。

My recommendation / 我的建议:

Start with `MapTripHistoryPanel` if the goal is to keep thinning `MapView.vue` with display-heavy derived data panels.
Start Network guided setup only if the goal shifts from decomposition to making provider setup easier for non-technical users.

如果目标是继续削薄 `MapView.vue` 且优先拆展示型派生数据面板，再做 `MapTripHistoryPanel`。
如果目标从拆分转为降低非技术用户配置供应商的难度，再做 Network 引导配置。
