...
| --- | ---: | --- |
| `src/views/ChatView.vue` | 6114 | still the largest maintainability hotspot; active-thread, Messages search/list, service-thread display, message-edit display-state, message action-sheet display-state, `+` panel-state, thread menu/settings draft-state, and pending quote display/action-state seams are now extracted |
