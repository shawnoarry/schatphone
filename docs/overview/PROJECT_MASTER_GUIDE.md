...
It should be treated as the current project-wide overview unless a more specific current document overrides a detail.

这是面向产品侧读者与后续 AI 工程师的主总览文档。
除非有更具体且当前有效的专题文档覆盖某个细节，否则应把本文视为项目级总说明。
...
1. 5-page layout baseline (pages 3-5 reserved for expansion).
   5 屏基线（3-5 屏为扩展预留）。
2. Widget and app tile organization with edit-mode gating.
   Widget 与应用入口组织，并带编辑模式门控。
...
7. 2026-05-02: added explicit reference to `FUNCTIONAL_CODE_NEXT_STEPS.md` as the post-visual-pause functional-code handoff.
   2026-05-02：新增对 `FUNCTIONAL_CODE_NEXT_STEPS.md` 的显式引用，作为视觉暂停后的功能代码接手单。
