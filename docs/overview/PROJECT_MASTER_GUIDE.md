...
- Book text-library V1 now exists as a separate app for long worldview documents, world rules, and encyclopedia text. `Settings -> WorldBook` remains the activation panel that links whole documents or selected Book sections into active world context. The first built-in Book content set, `现代首尔 K-pop 娱乐圈`, is available as callable read-only Book sources for the main worldview, world rules, and an encyclopedia placeholder; editing creates a user copy instead of changing the built-in template.
