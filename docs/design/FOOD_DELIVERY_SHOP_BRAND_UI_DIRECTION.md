...
- The implemented candidate presents the Chinese identity as `玉炉雅席` and keeps `Jade Hearth` in English. Built-in restaurant and twelve-dish copy follows system language as a presentation layer without rewriting user-authored records.
- Home now reads as a contemporary menu book with visible real chapters, stable dish sequence, table-oriented photography, vertical title slips, vessel-aware proportions, and rectangular `添菜 / Add dish` columns instead of a generic image-square/round-plus card.
- Menu opens the first real chapter, keeps bilingual search across all chapters, and repeats the same dish-leaf hierarchy rather than switching to the Dash Grill order-ticket pattern.
- Chapter and small-plate rails expose visible scrollbars and support touch swipe, wheel/trackpad, mouse drag, arrow controls, and keyboard navigation.
- Feast remains the defining special page and now presents non-fixed shared- and solo-table suggestions assembled from current menu sections, with every dish retaining detail and add actions.
- Product detail now uses generous `object-contain` photography, celadon framing, stable dish numbering, restrained ingredient annotations, and a sticky quantity/total action.
