...
说明：`classic` 更接近"基线"而非风格化美化包；风格化美化包目前指 `cloud-pastel` 与 `chromatic-glass` 两套（各含同名风格套件，`chromatic-glass` 套件另带 `liquid-prism` 配套组件集）。

### 2.2 候选风格族（未落地）

以下按参考方向归族。参考图仅定义方向气质；落地时所有资产必须 SchatPhone-owned 或自制，不得直接使用外部图库/社交平台素材（注意版权与水印）。

#### A. 月光手账族（Vintage Journal）— 候选 id `moonlit-journal` / 中文名「月光手账」

- 参考方向：暖香槟 + 米棕单色系；羊皮纸/信纸肌理；衬线西文标题；珍珠、链条、蜡封等复古手账元素；整体低对比、安静怀旧（参考图：产品 Home 的香槟复古稿）。
- 归类维度草案：材质=纸张/织物肌理；形状=中大圆角、细边框或无边框；阴影=极柔弥散影；文字=衬线西文 + 细系统中文；情绪=复古、静谧、温柔。
- 日夜策略：日间=香槟纸感（浅米底 + 深棕文字）；夜间=深咖啡底 + 烛光金/旧纸色 accent，须专项验证低对比下的可读性。
...
#### C. 贴纸涂鸦族（Outlined Sticker）— 候选 id `sticker-pop` / 中文名「贴纸涂鸦」

- 参考方向：粗黑描边 + 高饱和扁平色块 + 卡通贴纸造型；组件内大面积撞色（参考图：黑描边卡通 widget 包，仅作方向）。
- 归类维度草案：材质=扁平色块 + 描边；形状=中圆角 + 均匀粗描边；阴影=硬投影或无阴影；文字=圆润粗体；情绪=俏皮、活泼、直给。
- 与全局方向的张力：这是与 "iPhone-like 克制系统感" 距离最远的族。落地建议分两步：先以**图标包 + 配套 widget 集**试点（不动全局 shell token），验证气质后再决定是否升级为完整系统主题；即便升级，粗描边也应限定在图标与小组件，系统 chrome（通知、控制中心、列表）保持克制。
- 日夜策略：日间=奶油白底 + 撞色；夜间=深底 + 降饱和撞色，描边改浅色。
- 主要风险：全局化后破坏"在一部真实手机里"的沉浸错觉；描边与系统字排版的冲突。
...
落地顺序建议（2026-08-27 评估）：**月光手账族优先**——它主要落在色彩/字体/阴影 token 层，不需要大件新资产即可成立，且与产品既有的月光叙事（月光笺、Lunar Wanderer）天然契合，预期改动小、效果好；贴纸涂鸦族先以图标包 + widget 试点；奶油陶土族受肌理资产产能约束，最后排。此建议不构成执行队列，落地仍需走 roadmap 提升。
