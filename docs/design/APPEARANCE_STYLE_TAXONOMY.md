...
| `cream-shell` | 奶油软壳 | 奶油色块族 | `#FCEAD9` 奶油桃白为主体，统一平面奶油色块、奶白 Dock 与细线图标；提供可选内置柔光壁纸 |

说明：`classic` 更接近"基线"而非风格化美化包；风格化美化包目前指 `cloud-pastel`、`chromatic-glass`、`moonlit-journal`、`sticker-pop` 与 `cream-shell` 五套（各含同名风格套件，`chromatic-glass` 与 `cream-shell` 另带可选配套组件集）。
...
#### B. 奶油色块族（Cream Blocks）— id `cream-shell` / 中文名「奶油软壳」

状态：**已落地（2026-08-27）**。重新核对完整参考图后，确认画面中的软陶与厚重立体感主要来自真机外壳、机身边缘、拍摄光照和整机投影，不属于桌面主题本身；因此本主题归入平面奶油色块，而不是奶油陶土。

- 参考方向：`#FCEAD9` 奶油桃白、象牙白与极浅香草色为绝对主体；粉色只作为壁纸环境色，图标壳与 Dock 保持独立的香草奶油 / 象牙色，不吸收粉色成为实体表面。
- 归类维度：材质=平面奶油色块；形状=大圆角与胶囊；阴影=默认无阴影，仅允许为浅底辨识保留极轻边线或微小色差，禁止借用真机壳体的外投影、内高光和鼓包浮雕；文字=系统无衬线；情绪=柔软、干净、安静、轻甜。
- 日夜策略：日间=高明度奶油白；夜间=降低一档亮度的乳白与香草奶油，系统界面继续使用深色文字，不切换到深咖啡表面；Home 标签单独提高浅色可读性。
- 图标包策略：系统 App 共享近乎无阴影的奶油白圆角底，首批复用项目已有的简约功能 glyph，其余使用清晰的系统 glyph；Chat、商业 App 与单 App 自定义继续独立。
- 组件策略：同名套件可选安装 5 个平面奶油组件，只加入组件库，不自动摆到桌面、不随主题切换删除。
- 壁纸策略：提供一张原创的奶油桃白 / 淡粉柔光壁纸作为可选内置推荐。壁纸只承担低对比环境光与轻微质感，不模拟卡片、组件、Dock 或其他界面容器；套装应用时由用户决定是否替换，保留个人壁纸仍是合法组合。
- 主要风险：过度统一会让图标彼此难区分，因此通过细微 glyph 色相、清晰图形和文字标签维持辨识；浅色块在浅壁纸上应依赖边线和色差保持轮廓，而不是重新增加明显阴影。
...
落地说明（2026-08-27）：**月光手账**、**贴纸涂鸦**与平面奶油色块主题 **奶油软壳**均已落地。奶油陶土仍是独立候选，不得再从包含真机壳体与拍摄投影的参考图中反推桌面材质。此说明不构成执行队列，后续落地仍由用户请求、roadmap 与包交接共同确认。
