...
Status: `INTEGRATED_LOCAL / 23_SEOUL_PLACES_WITH_REVIEWED_MEDIA / 14_EXACT_CARD_HEROES / 9_DETAIL_ONLY_AREA_PLACES / 50_REAL_DERIVATIVES / 1_FICTIONAL_FALLBACK`

The 106-place Seoul catalog now resolves media through a separate V1 registry rather than adding image fields to canonical place records. Every place card has an image-backed overview slot with visible truth labeling and accessible alt text. Card heroes accept only exact photography, an explicitly labeled generated reconstruction, or category fallback; `area_atmosphere` is detail-only and the validator rejects it in `hero`. Reviewed records retain author/source/license/change attribution, and image-load failure returns to the labeled fallback. The runtime contains 50 real-photo derivatives across 23 Seoul places. Fourteen places have exact-photo card heroes. Nine places, including SM Entertainment HQ, have real surrounding-area media only in Place Details and therefore show the Seoul category fallback on the card. Times Square candidate B remains a user-approved exact detail slide; Hyundai Apgujeong still waits for its already approved additional detail candidate to transfer successfully.

`docs/design/MAP_PLACE_MEDIA_INVENTORY.md` records the fixed bespoke target as 113 built-in place decisions. Twenty-three Seoul places have some reviewed real-photo media, but only fourteen have card-eligible heroes; 92 Seoul hero decisions remain. The former 27-place area-atmosphere group now describes detail acquisition only. Extra gallery slides deepen a place but do not satisfy its card-hero requirement.
...
The ten-place follow-up now has nine additional reviewed `detail_gallery` slides. Two reuse complete local originals and seven use official Commons-generated renditions retained in the ignored source archive with rendition-byte SHA-256 and upstream file identity metadata. Eight derivatives were published and read back through batch `map-place-media-gallery-expansion-20260822`; the user-selected Times Square B derivative was separately published and verified through `map-place-media-times-square-gallery-20260822`. See `docs/qa/MAP_PLACE_MEDIA_GALLERY_EXPANSION_2026-08-22.md`. The media contract now strictly separates card identity from detail context: `area_atmosphere` is detail-only and the validator rejects it in `hero`. Nine area-only places, including SM Entertainment HQ, therefore show category fallback on the card while retaining their reviewed real surroundings in Place Details. Fourteen Seoul places currently retain exact-photo heroes.
...
27. Place media V1 keeps canonical place records unchanged while resolving an image-backed card asset for every place. The resolver and validator now enforce the overview/detail boundary: exact/generated/fallback may occupy `hero`, while `area_atmosphere` may occupy only `detail_gallery`. Twenty-three Seoul places have reviewed real-photo media, fourteen have exact card heroes, and nine retain their real area images only in Place Details while cards use category fallback. Every reviewed source page, author, license, source hash, runtime hash, alt text, truth grade, and disclosed crop/conversion remains in the registry/audit contract.
...
16. place media covers 23 Seoul places with 50 reviewed real-photo derivatives plus one fictional fallback; only fourteen currently have card-eligible exact heroes. Expanding hero coverage across the remaining 92 Seoul places, adding any generated reconstruction, user-selected Gallery media, periodic license revalidation, and true-device image/network evidence remain separate work.
