...
Status: `INTEGRATED_LOCAL / 66_SEOUL_PLACES_WITH_REVIEWED_MEDIA / 54_EXACT_CARD_HEROES / 12_DETAIL_ONLY_AREA_PLACES / 138_REAL_DERIVATIVES / 1_FICTIONAL_FALLBACK`

The 106-place Seoul catalog now resolves media through a separate V1 registry rather than adding image fields to canonical place records. Every place card has an image-backed overview slot with visible truth labeling and accessible alt text. Card heroes accept only exact photography, an explicitly labeled generated reconstruction, or category fallback; `area_atmosphere` is detail-only and the validator rejects it in `hero`. Reviewed records retain author/source/license/change attribution, and image-load failure returns to the labeled fallback. The runtime contains 138 real-photo derivatives across 66 Seoul places. Fifty-four places have exact-photo card heroes. Twelve places, including SM Entertainment HQ, Hongdae Street, Sanggye Jugong Apartment District, Acro River Park, Hannam The Hill, Namdaemun Pharmacy District, and London Bagel Museum Anguk, have real surrounding-area media only in Place Details and therefore show the Seoul category fallback on the card.

`docs/design/MAP_PLACE_MEDIA_INVENTORY.md` records the fixed bespoke target as 113 built-in place decisions. Sixty-six Seoul places have some reviewed real-photo media, and 54 have card-eligible heroes; 52 Seoul hero decisions remain. The former 27-place area-atmosphere group now describes detail acquisition only. Extra gallery slides deepen a place but do not satisfy its card-hero requirement.
...
The sixth 2026-08-23 selection round contributes thirteen reviewed derivatives across six runtime hero places. Samsung Town receives one exact hero and two explicitly area-level detail slides; JTBC Sangam Building and Shinsegae Department Store Gangnam receive exact heroes; SK Seorin Building receives one exact hero plus one area-level detail slide; Gangnam Station is promoted from detail-only coverage with a new exact `hero-v2` plus two exact detail slides while retaining its earlier area slide; and Megabox COEX receives one exact hero plus two exact detail slides. The thirteen public objects were published and read back with byte and SHA-256 verification through `map-place-media-batch-06-v2-20260823`; the `hero-v2` name preserves the historical Gangnam area derivative instead of replacing it. The user also confirmed visual directions for MBC Sangam Headquarters, CGV Yongsan I'Park Mall, Galleria Luxury Hall, and HYBE HQ, but their seven candidates remain local-only until reuse permission is verified and therefore do not enter runtime or the completion counts.

Batch 06 validation passes the focused media registry at 9/9, the asset registry at 1,057 tracked objects with zero violations, full Vitest at 312 files / 2,305 tests, production build, governance at 2 files / 14 tests, and the dedicated place-media Playwright at 2/2 across desktop Chromium and simulated Pixel 5. The full ESLint run passed before concurrent unrelated Food Delivery edits appeared; the final Map commit uses focused ESLint plus exact staged-file checks and leaves those unrelated errors and files untouched.
...
16. place media covers 66 Seoul places with 138 reviewed real-photo derivatives plus one fictional fallback; 54 currently have card-eligible exact heroes. Expanding hero coverage across the remaining 52 Seoul places, adding any generated reconstruction, periodic license revalidation, and true-device image/network evidence remain separate work.
