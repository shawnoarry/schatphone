...
Status: `INTEGRATED_LOCAL / 23_SEOUL_PLACES / 41_REAL_DERIVATIVES / 1_FICTIONAL_FALLBACK`

The 106-place Seoul catalog now resolves media through a separate V1 registry rather than adding image fields to canonical place records. Every place card has an image-backed overview slot with visible truth labeling and accessible alt text. Reviewed exact/area/generated records retain author/source/license/change attribution; remaining Seoul places use the approved CC0 Seoul category visual, fictional/player places use an explicit project-owned category fallback, and image-load failure returns to that same labeled fallback. The first pilot covers seven Seoul places. The first 2026-08-22 integration adds Gwanghwamun Square, Seoul City Hall, N Seoul Tower, Dongdaemun Design Plaza, Lotte World Tower, and Incheon Airport Terminal 1 with six overview heroes and eighteen extra detail slides. The follow-up expansion adds Gimpo International Airport, Gangnam Station, Seoul Express Bus Terminal, Yongsan Station, 63 Square, National Museum of Korea, Times Square Seoul, Lotte Department Store Main, Hyundai Department Store Apgujeong Main, and Olympic Park Seoul with ten additional overview heroes. Seven are exact-place photos and three keep conservative area-atmosphere labeling.

`docs/design/MAP_PLACE_MEDIA_INVENTORY.md` records the fixed bespoke target as 113 built-in place decisions. Twenty-three Seoul places are complete; 83 further Seoul places remain licensed-photo decisions, while the seven fictional places require project-authored generated reconstructions. Within Seoul, 79 places are exact-photo preferred and 27 are area-atmosphere preferred; truthful review evidence may downgrade or upgrade an individual target. Extra gallery slides deepen a completed place but do not increase this count.
...
429 with `Retry-After: 600` for the next request; no thumbnail or proxy copy was substituted.
...
27. Place media V1 keeps canonical place records unchanged while resolving an image-backed card asset for every place. Thirteen Seoul places now have reviewed media: the initial seven-place pilot plus a six-place batch with six overview heroes and eighteen extra detail slides. Remaining Seoul and fictional/player places use explicitly labeled category visuals, and every reviewed source page, author, license, source hash, runtime hash, alt text, truth grade, and disclosed crop/conversion is retained in the registry/audit contract.
...
16. place media covers 23 Seoul places with 41 reviewed real-photo derivatives plus one fictional fallback; expanding reviewed photography across the remaining 83 Seoul places, adding any generated reconstruction, user-selected Gallery media, periodic license revalidation, and true-device image/network evidence remain separate work.
17. the overview-versus-detail media roles are now established: one overview hero and an optional labeled detail gallery. A future source may still need distinct overview/detail crops after adaptation review. Separate product decisions are still required for a distinct long description and private user note; Event invitation placement and keyboard-initiated focus policy remain research items.
