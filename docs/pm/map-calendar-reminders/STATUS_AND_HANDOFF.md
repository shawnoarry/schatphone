...
Status: `INTEGRATED_LOCAL / 61_SEOUL_PLACES_WITH_REVIEWED_MEDIA / 48_EXACT_CARD_HEROES / 13_DETAIL_ONLY_AREA_PLACES / 125_REAL_DERIVATIVES / 1_FICTIONAL_FALLBACK`

The 106-place Seoul catalog now resolves media through a separate V1 registry rather than adding image fields to canonical place records. Every place card has an image-backed overview slot with visible truth labeling and accessible alt text. Card heroes accept only exact photography, an explicitly labeled generated reconstruction, or category fallback; `area_atmosphere` is detail-only and the validator rejects it in `hero`. Reviewed records retain author/source/license/change attribution, and image-load failure returns to the labeled fallback. The runtime contains 125 real-photo derivatives across 61 Seoul places. Forty-eight places have exact-photo card heroes. Thirteen places, including SM Entertainment HQ, Hongdae Street, Sanggye Jugong Apartment District, Acro River Park, Hannam The Hill, Namdaemun Pharmacy District, and London Bagel Museum Anguk, have real surrounding-area media only in Place Details and therefore show the Seoul category fallback on the card.

`docs/design/MAP_PLACE_MEDIA_INVENTORY.md` records the fixed bespoke target as 113 built-in place decisions. Sixty-one Seoul places have some reviewed real-photo media, and 48 have card-eligible heroes; 58 Seoul hero decisions remain. The former 27-place area-atmosphere group now describes detail acquisition only. Extra gallery slides deepen a place but do not satisfy its card-hero requirement.
...
16. place media covers 61 Seoul places with 125 reviewed real-photo derivatives plus one fictional fallback; 48 currently have card-eligible exact heroes. Expanding hero coverage across the remaining 58 Seoul places, adding any generated reconstruction, periodic license revalidation, and true-device image/network evidence remain separate work.
