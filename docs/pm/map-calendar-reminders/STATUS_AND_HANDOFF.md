...
Status: `INTEGRATED_LOCAL / 70_SEOUL_PLACES_WITH_REVIEWED_MEDIA / 58_EXACT_CARD_HEROES / 12_DETAIL_ONLY_AREA_PLACES / 145_REAL_DERIVATIVES / 1_FICTIONAL_FALLBACK`

The 106-place Seoul catalog now resolves media through a separate V1 registry rather than adding image fields to canonical place records. Every place card has an image-backed overview slot with visible truth labeling and accessible alt text. Card heroes accept only exact photography, an explicitly labeled generated reconstruction, or category fallback; `area_atmosphere` is detail-only and the validator rejects it in `hero`. Reviewed records retain creator/source/change attribution plus either reusable-license metadata or the explicit source-traced personal-project scope, and image-load failure returns to the labeled fallback. The runtime contains 145 real-photo derivatives across 70 Seoul places. Fifty-eight places have exact-photo card heroes. Twelve places, including SM Entertainment HQ, Hongdae Street, Sanggye Jugong Apartment District, Acro River Park, Hannam The Hill, Namdaemun Pharmacy District, and London Bagel Museum Anguk, have real surrounding-area media only in Place Details and therefore show the Seoul category fallback on the card.

`docs/design/MAP_PLACE_MEDIA_INVENTORY.md` records the fixed bespoke target as 113 built-in place decisions. Seventy Seoul places have some reviewed real-photo media, and 58 have card-eligible heroes; 48 Seoul hero decisions remain. The former 27-place area-atmosphere group now describes detail acquisition only. Extra gallery slides deepen a place but do not satisfy its card-hero requirement.
...
11. CJA-4 adds schema-V1 `store:activity-session`, one deterministic session per stable Agenda activity-step ID, absolute timestamps, explicit pause and completion policy, deterministic checkpoint reconciliation, owner-validated completion evidence, nested Calendar backup and rollback recovery, and startup/pageshow/visibility reconciliation. Agenda Journey hosts one quiet built-in Focus Companion with pause/resume, minimize/reopen, explicit completion, and no-event baseline behavior. CJA-5 then adds schema-V2 Activity Session owner validation plus one durable midpoint event family, introduced in Simulation V6 and carried by the current V7 schema, with `off | text` presentation and only a 0/2-minute timer result. CJA-6A adds only the documentation contract for a bounded, source-linked Narrative Timeline and read-only AI context; CJA-6B remains unimplemented. Travel steps fail closed, Map-owned duration is never copied, and Gallery backgrounds, Music/ambient callers, richer companions, appointment auto-entry, broader event families, and exact closed-app interaction remain absent. Focused CJA-4 plus persistence coverage passes 9 files / 65 tests; focused CJA-5/persistence coverage passes 10 files / 66 tests; the current bounded two-worker full suite passes 290 files / 2047 tests; desktop Chromium and simulated Pixel 5 Agenda/Activity Playwright passes 4/4 for CJA-4 and 4/4 for CJA-5. No physical-device evidence is claimed.
...
16. place media covers 70 Seoul places with 145 reviewed real-photo derivatives plus one fictional fallback; 58 currently have card-eligible exact heroes. Expanding hero coverage across the remaining 48 Seoul places, adding any generated reconstruction, periodic source review, and true-device image/network evidence remain separate work.
