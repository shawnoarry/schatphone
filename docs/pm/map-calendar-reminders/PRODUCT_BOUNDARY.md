...
The ordinary place-focus surface is one compact non-modal Map-owned overview/detail card anchored near the selected pin over existing truth. The overview identifies the place, gives a concise introduction and current-position relationship, and separates Journey intent from facility entry: an idle remote place offers Map Journey planning, a place viewed during a locked journey offers the existing journey without changing its destination, and a fixed entry slot remains present in every state. That slot is dim but focusable/clickable while remote and reports `当前不在设施附近` without emitting an entry request; exact stable current-position context or a measured position within the shared 30-meter entry radius activates `Enter`, and an inside session replaces it with `Leave`. Confirming nearby entry first snaps Map-owned manual evidence to the canonical place ID and coordinate, then creates the explicit inside session; it does not forge journey-arrival provenance. The idle role-position picker accepts a blank point directly or an existing place pin after confirmation, and a coincident role marker remains non-interactive below the place marker. Every overview has a stable image slot. Reviewed exact/area/generated media may support recognition, while category fallback remains explicitly non-evidentiary and cannot resemble verified place photography. The detail level remains about the place: fuller description, address, media truth/provenance/license, compact Share, reversible hidden-pin recovery, and a Settings handoff for player-owned management. It does not repeat Journey or entry actions. Map state, Footprints, global place-name language, arbitrary relocation, start-point mutation, and delete controls belong to Map Settings or their owning Map modules rather than the place card. Map persists explicit onsite/inside sessions, `manual` versus `journey_arrival` provenance, `Enter`/leave behavior, and bounded entry checkpoints without presenting those storage fields as place content. A place card shows no permanent Event entry, and distance outside the entry radius cannot reclassify a remote position as onsite.
...
Roadmap 4.14 keeps the large-map event-card lane separate from MJE-5. EVE-1 has landed the shared pure projection/explicit-host-registry Interface. EVE-2A froze the read-only current-place semantic overlay, `MapPlaceSessionCheckpointV1`, and a no-external-mutation validation Interface; EVE-2B completed Event-owned runtime/instance/text materialization; EVE-2C implemented Map provenance/session truth and exactly one registered host for the interior production-arrival-briefing archetype. Invalid/off-pack anchors remain reviewable without an invented position. `Expand event` is presentation/navigation only; every result returns through the source owner's Adapter. In local development only, an inside place may expose a clearly labeled ephemeral preview of that approved scene for UI testing without persona, journey, world-family, intensity, cooldown, or quota eligibility; preview choices remain page-local and create no Event Instance, trigger log, cooldown, quota use, or production invitation. Authored scene images may be referenced from Map/world packs, but Map does not own optional event text generation or later CG candidates.
...
Agenda Journey and `/agenda-journey` are implemented with a separate persistence owner and nested Calendar backup child. Activity Session is a separate time-only persistence owner with migration and nested Calendar backup child; its Focus Companion remains embedded in Agenda Journey rather than registered as another Home app. Simulation retains the CJA-5 event records and presentation policy without copying Activity Session. CJA-6B implements `生活志 / Chronicle` as a separate ordinary app and schema-V1 Diary Owner. Only user-authored diary entries are persisted there; the Narrative Timeline is a finite deterministic projection from currently verified typed owner references and is never stored as a second business record. Deleting a diary does not affect its sources, broken diary references retain the user's prose with unavailable state, and invalid projected sources fail closed. Chronicle cannot edit or infer Calendar, Agenda Journey, Map, Activity Session, Event Runtime, Work Hub, relationship, Wallet, Assets, or public truth. AI recall, automatic diary generation, broader event families, appointment auto-entry, media callers, richer companion state, interactive HTML, and CJA-6C remain unimplemented. The Schedule Orchestrator stays hidden and stores coordination state only.
