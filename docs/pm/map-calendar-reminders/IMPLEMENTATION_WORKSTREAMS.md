...
- a separate place-media registry keyed by `mapPackId + placeId`, with one stable image slot for every card; reviewed exact/area/generated media uses verified public derivatives, while missing/fictional/player places use an image-backed, explicitly non-evidentiary category fallback; detail keeps truth/source/license disclosure, accessible alt text, load-error fallback, and no mutation of canonical place records
...
- place focus is implemented as one accessible non-modal Map-owned floating overview/detail card anchored near the selected pin: the overview contains place identity, a stable image slot, concise introduction, current-position relationship, a separate `Go`/journey action, Details, and one permanently reserved entry slot. Remote entry stays dim but clickable and explains that the role is not near the facility; exact or measured-within-30-meters onsite context activates `Enter`, and inside replaces it with `Leave`. The idle role-position picker accepts either a blank point or a confirmed existing place pin, while coincident role markers remain below and cannot intercept place interaction in either renderer. The detail contains place description, address, media truth/provenance/license, compact Share, hidden-pin recovery, and a Settings handoff for player-owned management without repeating Journey or entry actions. It does not expose Map state, Footprints, global language preferences, arbitrary relocation, start-point mutation, deletion, a permanent Event placeholder, a nested modal, a new Store, or inferred legacy visit history, and it does not mask or lock the surrounding map
- the presence slice is implemented with one shared 30-meter entry radius, canonical-place snapping for confirmed nearby manual entry, exact inside-session validation, manual-versus-journey provenance, and persistence; event invitations remain absent until an eligible inside-session projection exists
- the restricted-place slice keeps that same entry/session surface while adding optional authored place concepts, exact confirmed-world and Work Hub evidence resolution, one stable Event Instance V2, one Map owner fact, durable routine/reviewed reuse, fail-closed Settings guidance, and a direct legacy path for unannotated places; no genre branch or model call runs during entry
- local development provides one explicitly labeled, non-persistent event preview after entering any place so presentation and three-choice QA do not require persona, journey, world-family, intensity, cooldown, or quota setup; it creates no runtime instance/log and is not mounted in production
...
- Mail and accepted Work Hub S1 proposals reuse one source resolver registry, one Calendar editor, and one idempotent creation path; Work Hub acceptance remains source-owned and zero-write, linked state is read-only from Calendar, and unaccepted/declined/forged source IDs fail closed
