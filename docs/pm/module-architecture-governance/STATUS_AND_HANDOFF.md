...
- `FOUR-SHARD RELEASE GATE LOCAL_DONE 2026-08-31`: PR CI and Pages separate audits/lint/unit/build from a four-entry Playwright matrix. Every shard uses one worker and zero retries, validates skipped/unexpected/flaky JSON stats even after a failed test step, and uploads shard-specific diagnostics. Pages deploy depends on both the exact-commit `dist` build and the complete E2E matrix. Local proof is 422 passed / 4 accepted skips / 0 failed; exact remote confirmation remains pending the follow-up push;
...
- first successful Chat activation and the explicit custom-role-to-Chat journey are completed product evidence rather than architecture prerequisites; GitHub Pages has direct user-configured provider model discovery, connection, one real Chat reply, reload persistence, and real-provider restricted-relay evidence through both deployed Vercel and Cloudflare backends. Automated hosted-browser PWA/offline and backup recovery are remotely proven; OS-installed PWA, external protection, and named true-device proof remain release work.
- `REMOTE_DONE 2026-09-01`: roadmap 4.9's release-local backup contract remains complete and unchanged. Exact revision `8d38dfa8c62a60687407d2e61659d9078cc023bc` passed Pages run `33504446970`, automatic Hosted Product Proof run `33505769229`, and the later non-overlapping manual CI run `33505937702`; the live release marker matched the same SHA. Desktop Chromium and simulated Pixel 5 proved complete export, blank-storage restore, and reopen with the completed recovery journal. Do not reopen persistence architecture or personal R2 from this result; OS installation, external policy, and physical-device evidence remain separate release gates.
...
- the 2026-08-22 ChKSz access-diagnosis and quality-fallback follow-up passes 4 focused Music files / 61 tests, lint, the full 303-file / 2186-test Vitest suite, production build across 594 modules with only the existing chunk-size warning, governance at 2 files / 14 tests, all 12 desktop/simulated-Pixel-5 `music-app.spec.js` cases, the 16-case visual gate, and `git diff --check`. Search rows expose conservative `VIP` / `Purchase` / `Limited` hints without disabling Play; selected high quality falls back exactly once to the platform baseline only for missing-URL or explicit quality/format failures; authentication, quota, rate-limit, provider-unavailable HTTP, and network failures do not fall back. Cached-stream recovery remains bounded even when each resolution uses two quality attempts;
...
8. `CMG-05 DONE 2026-08-21`: implementation `d8662cb` carries one stable gift experience ID through Shopping, Chat, Reminder/Calendar, Wallet, Phone, and Relationship owner records. Ordering, delivery, and recipient feedback update one concise role memory; supporting progress and retries do not add relationship metrics. Old gift orders derive the ID when read, existing `shopping_gift__*` memories receive later progress without migration, and Phone rejects same-name cross-contact gift matching when a stable recipient identity exists. Focused Vitest passed 10 files / 100 tests; full Vitest passed 301 files / 2124 tests; dedicated desktop/simulated-Pixel-5 Playwright passed 2/2; lint, build, governance, and diff checks passed. No shared-experience Store, owner-body copy, retention-cap change, or legacy-memory deletion was introduced.
9. `CMG-06 DONE 2026-08-21` at `f9f14f9`: the controller implementation removes Relationship Runtime's global 500-event and 300-entity writer/restore slices, adds the v1 -> v2 carrier reader and full-state rollback, and gives Contacts page/offset memory projections with source filters while keeping Chat prompt recall bounded. Focused Runtime/Contacts coverage passes 31/31; Contacts Chromium E2E passes 1/1; full Vitest passes 301 files / 2129 tests; lint, build, governance, and diff checks pass. Existing v1 rows are preserved; data already discarded by older builds is not claimed recoverable. No Event Instance 240-row or Mini Scene 120-artifact limit changed.
10. `CMG-07 DONE 2026-08-21` at `d8b46fc`: the local implementation removes Event Instance V2's global 240-row writer slice, registers Simulation storage V7 with V1-V6 migration, and preserves the full instance array through 241+ active/terminal rows, close/reopen, complete backup restore, and save-failure rollback. A long-running address-change Service Case remains inspectable; event logs and other bounded collections are unchanged. Focused Simulation/Event Runtime coverage passes 19/19 across 3 files; full Vitest passes 301 files / 2132 tests; lint, production build, governance, and diff checks pass. Documentation handoff follows in a separate local commit.
...
2. `DONE 2026-08-19`: persist the current shell's bounded artifacts/policies/bindings/audit through `store:mini-scene` V1 and include them in complete-backup v4 plus rollback/crash recovery while retaining v3 manifest verification. `CMG-08` must change full-scene retention to an explicit user choice without moving event-result or memory ownership into Mini Scene;
3. `DONE SHELL 2026-08-19`: register Event Runtime as the only functional caller, require an injected AI provider and exact HTML-free structured Draft with provider provenance, fail closed without a deterministic substitute, and expose a global Text Presenter plus bounded owner-validation choice request. The shell's commit-before-present behavior is transitional until `CMG-08` adds temporary presentation plus explicit full-scene retention;
