...
- the executable canonical inventory independently classifies 17 persisted stores, the serialized mirror, Gallery binary storage, image-generation credential/candidate/legacy carriers, TTS device configuration/credentials, the Home local hint, Chat session feedback, and their logical owner/data classes; Contacts-in-Chat and WorldBook-in-System remain explicit rather than inheriting the physical store owner;
