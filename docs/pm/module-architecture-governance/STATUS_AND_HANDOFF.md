...
- the repository now contains a locally validated Cloudflare Worker/static-assets deployment path: `npm run build:cloudflare` builds at `/`, the Worker serves the SPA and the same two fixed proxy routes, unknown API routes fail as JSON `404`, and Wrangler dry-run detects the `ASSETS` binding; the first Git-connected Cloudflare deployment and resulting URL remain pending;
...
4. first Chat activation and the explicit custom-role-to-Chat journey are already product-side complete; remote CI/Pages, the deployed `/schatphone/` smoke, and the Git-connected Vercel root/proxy infrastructure baseline are complete. The Cloudflare Worker/static-assets path is locally ready but still needs its first Git-connected deployment; deployed PWA/install/offline, configured hosted-provider Chat, and named true-device backup evidence still close the public-release gate;
