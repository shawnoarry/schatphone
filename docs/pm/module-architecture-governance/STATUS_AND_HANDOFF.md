...
- None after the 2026-06-19 Chat active thread read-model extraction. Next safe governance candidate: continue ChatView decomposition with one adjacent panel-state or service-thread display read-model seam, but only after promoting a concrete slice here and in `docs/roadmap/TODO_ROADMAP.md`. Alternative: move to Contacts/WorldBook top-hotspot seams if Chat work should pause.
