...
- roadmap 4.5-CMG now governs the confirmed shared-experience, role-memory, Event Instance, Mini Scene, and persistence-result defects through fixed IDs. `CMG-00` through `CMG-05`, `DCF-03`, and `DCF-05` are complete; `CMG-06` is next in order but has not started, while `CMG-07`, `CMG-08`, and `DCF-06` require their own exact non-overlapping reservations;
...
| Next dependency-safe items | `CMG-06` is next in order; `CMG-07`, `CMG-08`, and `DCF-06` are separately assignable only after new exact non-overlapping reservations |
| Active item | none; `CMG-05 DONE 2026-08-21` |
...
| Source branch, base, and worktree | `main`; reservation base `bfb774f4bed82dd1b3ec0d2b289344aef9e5cce6`; behavior commit `d8662cb`; `D:\github\schatphone` |
| Existing dirty/untracked inventory | User-owned Map work is preserved in local commits `8a67852` and `bc92379`; unrelated `tmp/**` experiments remain untracked and were not staged. |
| Risk lane | Completed as additive gift-link fields and one Phone form choice only. No shared-experience Store, owner-record body copy, legacy `shopping_gift__*` migration/deletion, retention-cap change, or supporting-progress metric stacking was introduced. |
| Implemented paths | `src/lib/{shared-experience-contract,relationship-fact-adapters,reminder-cues}.js`; `src/stores/{shopping,reminders,calendar,chat,wallet,phone,relationshipRuntime}.js`; `src/views/{ShoppingView,PhoneView}.vue`; focused Store/Adapter/View/runtime tests; `tests/gift-shared-experience-runtime.test.js`; `e2e/shopping-life-consequence.spec.js` |
| Acceptance | Met. One gift order creates one stable experience ID. Shopping order/updates, Chat notification, Reminder/Calendar record, Wallet transaction, linked Phone feedback, and Relationship facts keep their own records while referencing that ID. Relationship review exposes one evolving memory with ordered reserved/delivered/feedback progress; retries create neither a second experience nor duplicate metrics. Existing records without the new field and legacy `shopping_gift__*` memories remain readable and are not migrated or deleted. Same-name contacts cannot borrow a gift that is bound to another stable contact identity. |
| Validation | Focused Vitest passed 10 files / 100 tests; full Vitest passed 301 files / 2124 tests; targeted `shopping-life-consequence` Playwright passed 2/2 on desktop Chromium and simulated Pixel 5; lint, production build, governance at 2 files / 14 tests, and diff checks passed. The first default-worker full run exposed one unrelated timing failure in `persistence-write-result.test.js`; that file passed alone at 13/13, the two-worker full suite passed, and the final default-worker full suite passed. Existing jsdom audio/canvas notices remain non-failing. |
| Integration state | `CMG-05 DONE 2026-08-21`; behavior commit `d8662cb` on local `main` |
...
`CMG-05` is complete at `d8662cb`. `CMG-06` is next in roadmap order, but removing the 500-row cap must begin as a new exact storage/migration/rollback reservation and must include pagination and long-run proof; it is not authorized as an incidental follow-up to this gift slice. `CMG-07`, `CMG-08`, open direct fixes `DCF-01`, `DCF-02`, `DCF-04`, and test repair `DCF-06` may run independently only after exact non-overlapping paths and bases are recorded.
