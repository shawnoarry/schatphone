...
| Risk lane | Remove only Event Instance V2's global 240-instance retention. Preserve event logs, Owner Facts, Activity Session records, Mini Scene's 120-artifact limit, owner-native records, backup section shape, and transactional rollback. Add a v6 -> v7 local carrier reader; rows already discarded by older builds remain explicitly unrecoverable. |
