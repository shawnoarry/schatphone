...
- roadmap 4.5-CMG now governs the confirmed shared-experience, role-memory, Event Instance, Mini Scene, and persistence-result defects through fixed IDs. `CMG-00` through `CMG-06`, `DCF-03`, and `DCF-05` are complete; `CMG-07`, `CMG-08`, and `DCF-06` require their own exact non-overlapping reservations;
...
| Next dependency-safe items | `CMG-07`, `CMG-08`, and `DCF-06` remain separately assignable only after new exact non-overlapping reservations |
| Active item | none; `CMG-06 DONE 2026-08-21` |
...
| Source branch, base, and worktree | `main`; CMG-06 reservation base `d1cfbab179e3197e9823e50f7cd73b1b978d23e9`; controller changes are in the current local commit; `D:\github\schatphone` |
| Existing dirty/untracked inventory | User-owned Map work is preserved in local commits, including `d1cfbab`; unrelated `tmp/**` experiments remain untracked and must not be staged, reset, or cleaned. |
| Risk lane | Remove only Relationship Runtime's global 500-event and 300-entity retention. Preserve owner records, legacy `shopping_gift__*` memories, existing 240/120 limits, backup section shape, and transactional rollback. Add a v1 -> v2 local carrier reader; rows already discarded by older builds remain explicitly unrecoverable. |
| Reserved paths | `src/stores/relationshipRuntime.js`; `src/composables/useContactsMemoryListModel.js`; `src/views/ContactsView.vue`; `tests/relationship-runtime-store.test.js`; `tests/contacts-memory-list-model.test.js`; this handoff, Contacts handoff/README, and `docs/architecture/RELATIONSHIP_GROWTH_EVENT_SYSTEM.md`. |
| Acceptance | Complete fact/entity arrays survive 501/301 rows, close/reopen, complete backup restore, and v1 carrier migration; save failure restores the full prior state; Contacts reads one page with source filters and total/page metadata; Chat prompt recall stays within item/character budgets. |
| Validation | Focused Relationship Runtime and Contacts memory coverage 31/31 pass; Contacts Chromium E2E 1/1 passes; full Vitest passes 301 files / 2129 tests; lint, production build, governance, and `git diff --check` pass. |
| Integration state | `CMG-06 DONE 2026-08-21`; controller changes are committed on local `main` |
