...
- None after the 2026-06-19 Settings push workflow extraction. The next governance slice should move out of residual Settings work unless a specific Settings bug appears. Recommended next candidate: start Phase 2 large-view decomposition with one `ChatView.vue` route/thread/read-model or panel-state composable, promoted into this file and `docs/roadmap/TODO_ROADMAP.md` as `IN_PROGRESS` before implementation. Alternative safe candidate: a narrow `systemStore` facade such as backup-reminder settings if the team wants a store-interface slice instead of a view slice.
