...
- Music's module slice is implemented behind dedicated contract, ChKSz Adapter, cross-module Interface, playback runtime, Music-local binary storage, and Pinia Store files. The ChKSz path supports NetEase/QQ/Kugou search, on-demand stream resolution, NetEase lyrics/playlist intake, quota/error handling, and a fixed official endpoint while keeping keys device-local and stream URLs runtime-only. Direct HTTPS URLs and local-file metadata normalize into the public library; imported blobs use the separate `schatphone-music-media` IndexedDB carrier, resolve to revocable object URLs only at Play, and remain excluded from ordinary backup. Public library/provider/integration state is required through `system-settings`; `schatphone:music:credentials` remains a separately inventoried device-local secret carrier. The first Chat caller uses a bounded track payload plus the excluded 24-hour internal-share draft, while confirmed Chat history stores only the normalized source card. Chat/Map routes and projections omit credentials, endpoint/header data, raw responses, queue contents, stream URLs, and local media IDs;
...
- 41 route-view files, 18 Pinia stores, 51 Vue components under `src/components`, and 37 JavaScript composables;
- 173 JavaScript files, 93 Vue files, and 171,987 source lines under `src` in the current worktree;
- 214 static unit-test files;
- the 2026-08-09 current Music + ChKSz + direct/local intake worktree passes diff check, governance, lint, 217 Vitest files / 1549 tests, production build, 8 focused Music Playwright cases across desktop and simulated mobile, and the 12-case default/zen system visual gate; direct browser inspection also passes at desktop and 393 x 852 with no document/settings overflow, while the existing MapLibre/jsdom canvas messages remain non-failing test-environment notices;
...
- `src/lib/music-contract.js`, `src/lib/chksz-music-adapter.js`, `src/lib/music-module-interface.js`, `src/lib/music-playback-runtime.js`, and `src/stores/music.js` own Music normalization, generic JSON and dedicated ChKSz provider behavior, browser playback, persistence facade, and bounded Chat/Map seams;
