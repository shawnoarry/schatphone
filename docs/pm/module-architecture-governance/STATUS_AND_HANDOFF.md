...
- roadmap 4.5's repository-asset externalization slice is complete. All 771 migration objects and the three-object project-brief smoke are verified and registered; the authenticated batch endpoint is deployed; the long-lived upload/list-only project credential is configured per publishing workstation; runtime URLs use centralized public `schatphone-assets/` construction; and approved generated-asset plans now publish automatically in pre-commit, stay offline when no plan exists, and use an exact credential-free Git fallback when publication is temporarily unavailable so another PC can continue. Successful publication re-downloads and verifies bytes, stages the registry, and removes the local/fallback generated files. The original 975 planned migration/poster files were copied, size/SHA-256 verified, then removed from the repository through separate manifests in device-local home-PC archive storage whose path is not project configuration. PWA/install/offline resources and files outside the approved plans remain local. Commits `f654b81` and `ffae433` are pushed, GitHub Pages Run `31537206567` plus Vercel and Cloudflare deployment checks passed, all three production hosts loaded 18/18 observed image-bed images without browser errors, and the temporary migration token is revoked while the project publisher remains active;
