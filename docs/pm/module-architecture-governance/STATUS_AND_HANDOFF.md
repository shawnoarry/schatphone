...
- roadmap 4.5-CMG now governs the confirmed shared-experience, role-memory, Event Instance, Mini Scene, and persistence-result defects through fixed IDs. `CMG-00` through `CMG-04` and `DCF-05` are complete; `CMG-05` is reserved on PC-A from synchronized base `bfb774f`, while `CMG-08` or `DCF-06` may be claimed only on paths outside its exact reservation;
...
| Field | Value |
| --- | --- |
| Plan baseline | `CMG-00 DONE 2026-08-20` at `fef7989`; `CMG-01 DONE 2026-08-20` at `73672df`; `CMG-02 DONE 2026-08-20` at `208e1dc`; `CMG-03 DONE 2026-08-20` at `86270d8`; `DCF-05 DONE 2026-08-20` at `f140557`; `DCF-03 DONE 2026-08-21`, behavior at `42742e5` and regression evidence at `e9607c0`; `CMG-04 DONE 2026-08-21` at `134f7f7`; synchronized reservation base `bfb774f` |
| Next dependency-safe items | `CMG-05` is active; `CMG-08` and `DCF-06` remain separately assignable only outside the reserved paths below |
| Active item | `CMG-05 IN_PROGRESS` |
| Integration controller PC | `SKY-20250212UBG` |
| PC-A physical machine / role | `SKY-20250212UBG` / integration controller and `CMG-05` executor |
| PC-B physical machine / role | `UNASSIGNED` |
| Source branch, base, and worktree | `main`; exact base `bfb774f4bed82dd1b3ec0d2b289344aef9e5cce6`; `D:\github\schatphone` |
| Existing dirty/untracked inventory | User-owned Map work in `docs/design/MAP_PLACE_DETAIL_UI_REVIEW.md`, `e2e/map-place-media.spec.js`, `src/components/map/MapPlaceFocusSheet.vue`, `src/views/MapView.vue`, `tests/map-place-focus-sheet.test.js`, `tests/map-view-information-architecture.test.js`, and `output/e2e/map-place-media/**`; unrelated `tmp/**` experiments. Preserve and never stage these files. |
| Risk lane | Additive gift-link fields and one Phone form choice only. Do not add a shared-experience Store, copy owner record bodies, migrate/delete legacy `shopping_gift__*` memories, change retention caps, or stack relationship metrics for supporting progress. |
| Reserved paths | `docs/roadmap/TODO_ROADMAP.md`; `docs/pm/module-architecture-governance/STATUS_AND_HANDOFF.md`; `src/lib/shared-experience-contract.js`; `src/lib/relationship-fact-adapters.js`; `src/stores/{shopping,reminders,calendar,chat,wallet,phone,relationshipRuntime}.js`; `src/views/{ShoppingView,PhoneView}.vue`; `tests/{shared-experience-contract,relationship-fact-adapters,relationship-runtime-store,shopping-store,shopping-view,reminders-store,calendar-event-store,phone-store,phone-view}.test.js`; `tests/gift-shared-experience-runtime.test.js`; `e2e/shopping-life-consequence.spec.js` |
| Acceptance | One gift order creates one stable experience ID. Shopping order/updates, Chat notification, Reminder/Calendar record, Wallet transaction, linked Phone feedback, and Relationship facts keep their own records while referencing that ID. Relationship review exposes one evolving memory with ordered reserved/delivered/feedback progress; retries create neither a second experience nor duplicate metrics. Existing records without the new field and legacy `shopping_gift__*` memories remain readable and are not migrated or deleted. |
| Required checks | Focused Vitest for the shared-experience projection, owner Store normalization, adapters, Shopping/Phone views, and Relationship Runtime; targeted `shopping-life-consequence` Playwright on desktop Chromium and simulated Pixel 5; `npm.cmd run lint`; full `npm.cmd run test`; `npm.cmd run build`; `npm.cmd run governance:check`; `git diff --check`. No dependency audit unless the lockfile changes. |
| Integration state | `CMG-05 IN_PROGRESS`; no behavior commit yet |
| Remote synchronization | Reservation begins from `origin/main == bfb774f`; push this reservation commit before behavior edits |
...
`CMG-05` is active on PC-A from `bfb774f`. Do not begin `CMG-06` or remove the 500/240/120 caps before `CMG-05` and the named persistence, migration, rollback, pagination, and long-run gates are accepted. Open direct fixes `DCF-01`, `DCF-02`, `DCF-04`, and test repair `DCF-06` may run independently on PC-B only after exact paths are reserved outside this slice and the controller confirms a non-overlapping base.
