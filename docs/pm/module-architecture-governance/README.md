...
- roadmap 4.5-CMG is the active cross-PC governance plan for shared experiences, role memory, Event Instance and Mini Scene retention, confirmed persistence, and the audited direct defects. DCF-01's WorldBook multi-entry navigation repair is complete at `4654e02`; `TODO_ROADMAP.md` owns every item status, and this package owns the current execution handoff without duplicating the ledger;
...
- Music's module slice is implemented behind `music-contract.js`, `chksz-music-adapter.js`, `music-module-interface.js`, `music-playback-runtime.js`, `music-local-media-storage.js`, `music-provider-cache.js`, and `stores/music.js`: primary user-authorized ChKSz search/on-demand resolution, supplementary generic JSON, and the no-key Radio Browser HTTPS/MP3 live-station preset share one normalized boundary. ChKSz supports broader metadata and conservative rights-hint variants, one bounded basic-quality fallback when a selected high quality has no URL, expiry-aware memory-only stream reuse for up to 7 days with same-track single-flight and a 50-track LRU limit, plus a separate 30-day rebuildable IndexedDB cache for normalized metadata and lyrics; no API Key, resolved stream, or audio byte enters that database. Provider-declared expiry shortens playback reuse with a 5-minute safety margin, while cached playback failures invalidate the stream and trigger at most one new resolution. Authentication, quota, rate-limit, provider-unavailable HTTP, and network failures never trigger quality fallback. Direct HTTPS URLs persist as Music tracks; imported local binaries use a separate Music-owned IndexedDB carrier and runtime-only object URLs. Public library/provider/import metadata is backup-covered through System settings, while API keys, local audio binaries, and rebuildable provider cache remain device-local backup exclusions. Chat/Map receive bounded references/projections without stream URLs, media IDs, raw queues, or provider secrets. The first Chat caller converts a track payload into a source-owned card through one transient internal-share draft; card return opens details without playback. The first Map caller consumes bounded now-playing/quick-track/station projections during an active journey, while Music retains radio queue, audio runtime, and global floating-player ownership;
