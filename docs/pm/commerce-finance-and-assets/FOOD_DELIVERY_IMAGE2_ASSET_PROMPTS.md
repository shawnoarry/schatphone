...
- 本节的多商户摄影仍是跨机器续接合同；当前已交付平台层的透明骑手返工、周末横幅文案版、周末活动海报、十一家完整五图菜单、两张新增摄影封面、四张透明商户 Logo、七张摄影型商户订单标识和十张透明分类图标。请求、候选、正式副本、显式版本选择与验收记录位于 `output/imagegen/baemin-platform/`。
...
2. 11 家商户的 `01..05` 五图菜单均已生成、接受并接入；新增 36 张由 `accepted/integration-selection-v1.json` 固定候选版本并导出为 `768x768` 正式副本。
