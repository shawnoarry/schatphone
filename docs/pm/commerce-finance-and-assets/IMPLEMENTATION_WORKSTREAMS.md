...
- Shopping's built-in pseudo-folder exposes eleven independent real-name retail-reference Apps: five marketplace-style Apps (Coupang: `schat_mall` / `city_market`; 29CM: `nova_digital` / `tech_catalog`; Kurly: `daily_fresh` / `fresh_market`; E-MART TRADERS: `traders_club` / `member_warehouse`; MUSINSA: `musinsa_style` / `fashion_catalog`) and six specialty-store Apps (WORKSOUT: `style_cloud` / `fashion_editorial`; IKEA Korea: `nordhus_home` / `room_planner`; OLIVE YOUNG: `mellow_care` / `care_lab`; CU: `cu_24` / `neighborhood_convenience`; BOONTHESHOP: `boon_select` / `buyer_atelier`; Galleria Luxury Hall: `galleria_luxury` / `luxury_hall`). Their routes, brand-derived headers, Seoul setting anchors, catalog views, search, categories, favorites, carts, checkout, order views, and logistics views are App-specific. All eleven reuse one Shopping-owned schema, quote, backup, and persistence implementation underneath, with visible state scoped by `serviceKey` and no cross-App cart or mixed checkout.
- `SHOPPING-STOREFRONT-UI-03`: all eleven storefronts add presentation-local behavior without widening the shared contract. The original six retain Coupang review/fulfillment confirmation, 29CM editorial mode and issue index, Kurly fulfillment lanes, WORKSOUT Drop/Lookbook, IKEA room tones, and OLIVE YOUNG routine builder; the five extensions add TRADERS Pallet/List plus member campaign wall and pack focus, CU Shelf/Quick Pickup plus scene promos and pickup board, WORKSOUT campaign/lookbook/style-note rails, MUSINSA campaign wall/lookbook chapters, BOONTHESHOP buyer stories/material desk/fitting-room rail, and Galleria private campaigns/hall map/service strip. These states are intentionally ephemeral and cannot create cross-store business records.
- `SHOPPING-STOREFRONT-UI-04 / DONE 2026-08-21`: the shared long-page depth model is replaced by store-owned page families over the same Shopping business kernel. All eleven Apps have independent `shopView` vocabularies, route-backed paginated Category/Collection pages, full PDPs, focused Cart/Bag, Checkout Review, Orders, Order Detail, Logistics, and Service/Help pages, plus storefront-specific decision modules for fulfillment, editorial selection, cold chain, size/fit, room measurements, routines, warehouse value, pickup, fitting, and concierge service. Desktop Chromium and simulated Mobile Chrome pass the full ten-case Shopping storefront suite. Formal media packs, deeper campaign/editorial routes, and named physical-device acceptance remain separate follow-ups rather than unfinished UI-04 architecture.
- Built-in Shopping content currently includes 100 stable bilingual fictional products. The prior 57 stable records remain unchanged, and 43 additive products deepen thin categories across all eleven storefronts so every store has 8-10 visible products and every allowed category remains populated; `mall` retains its aggregate all-products behavior. Normal hydration may add missing built-in IDs without overwriting same-ID saved products; explicit backup restore must remain snapshot-faithful.
