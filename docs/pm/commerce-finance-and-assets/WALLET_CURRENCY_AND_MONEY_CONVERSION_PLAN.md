...
| Currency registry | Built-ins now carry exponent metadata; injected/custom currencies accept an exponent and legacy definitions safely default to exponent 2 | Add an explicit exponent control to the World Pack currency editor before claiming complete custom-currency authoring |
| Shared money and quote core | Wallet exposes integer-minor-unit quotation and formatting backed by decimal-string rates and deterministic `ROUND_HALF_UP` arithmetic | Adopt it module by module without reinterpreting legacy exponent-2 `*Cents` fields |
...
| Food Delivery setting link | Food Delivery watches the Wallet primary currency | The link exists, but most menu values are relabeled rather than numerically converted |
...
| Platform minimum order | Some Baemin merchants use hard-coded strings such as `10,000원` while other values say CNY | Replace strings with structured money and one quote/format path |
| Shopping | Products and orders keep source currencies | Add primary-currency quotations without replacing Shopping source truth |
| Historical records | Existing order and Wallet currencies remain persisted | Preserve this behavior and add explicit quote snapshots for new records |
...
Foundation checkpoint, 2026-08-06: `src/lib/currency-system.js` and `src/stores/wallet.js` implement the shared registry metadata, decimal-string rate revisions, exact minor-unit quote, deterministic formatting, explicit missing-rate result, backup compatibility, and Wallet service API. Direct tests cover exponent-0/exponent-2 formatting, positive and negative conversion, half-up boundaries, unavailable rates, revisions, and legacy rate migration. This checkpoint does not migrate general Food Delivery, Baemin, Shopping, Chat transfer persistence, orders, refunds, or ledger records to quote snapshots.
...
1. `FOUNDATION DONE 2026-08-06`: add `exponent` to built-in and custom currency definitions with safe legacy defaults; explicit World Pack exponent authoring remains a UI follow-up.
...
4. Correct Baemin hard-coded minimum-order strings and use Food Delivery as the first complete module adopter.
5. Migrate Shopping and Chat transfer presentation after Food Delivery proves the contract.
