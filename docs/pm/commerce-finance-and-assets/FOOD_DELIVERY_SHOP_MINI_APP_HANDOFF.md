...
- The accepted single-item pack must not be overwritten for the combo correction. Detail currently labels item `01` and `02` as the main component and code-renders the selected side/drink tray map. Two additional square default-combo photographs are required: `dash-grill/combos/dash-grill-double-stack-combo-01.png` and `dash-grill/combos/dash-grill-golden-chicken-combo-01.png`. Each must show the fixed burger/chicken stack, Sea-Salt Fries, and Fountain Cola as one complete quick-service tray, keep all three subjects readable at card and detail sizes, and retain Dash Grill's paper/mustard/ink/red photography language. Status: `NOT_GENERATED / NOT_CONNECTED`; the new tray-ticket UI is `IMPLEMENTED / USER_VISUAL_ACCEPTANCE_PENDING`.
...
Deferred shop UI decisions are recorded rather than partially implemented. Jade Hearth needs one integrated pass beyond product cards: paper-banquet-menu rhythm, course order, vertical title slips, dishware ratios, and a clear `加菜 / Add dish` column should be solved together with its wider UI issues. Verdant Day keeps circular food photography but still needs ingredient-level salad customization: each ingredient must own explicit plus/minus controls and a visible price delta, which requires a modifier snapshot contract before UI work. Dash Grill now has connected image-led order tickets on Home/Menu plus a tray-ticket combo detail. Selection pricing and Bag/checkout/order snapshots are implemented; user visual acceptance and the two complete-tray photographs remain pending.
