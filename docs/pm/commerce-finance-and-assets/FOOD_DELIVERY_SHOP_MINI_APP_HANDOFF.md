...
- Peach Cloud now has a dedicated Vue app branch instead of conditional styling on the shared shop template. Its full-width petal-pink, pink-mist, pale-green, and charcoal app frame, five home category shortcuts, product rails, promotion, fixed bottom navigation, and route-driven Search, New, Bag, Orders, and order-detail pages deliberately contrast with Moon Bistro's dark side-rail tray menu. This palette is a later Peach Cloud brand decision layered over the Figma-derived information architecture; it does not change the Figma evidence or shared order runtime.
