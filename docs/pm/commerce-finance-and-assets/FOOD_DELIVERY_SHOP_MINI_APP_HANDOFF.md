...
- A one-poster dynamic-price pilot adds a versioned White Peach Lime derivative beside that formal pack. Only its baked price field is cleared; Home and Discover/New render `26 CNY`, `3.34 EUR`, or `5015 KRW` from the stable menu item plus Wallet settings in an independent percentage slot. The original White Peach Lime poster remains unchanged, the other two posters retain baked prices, and the derivative is excluded from the current `39`-asset formal count. Prompt, mask, candidate, accepted composite, pixel invariant, and visual-QA decisions are retained under `output/imagegen/peach-cloud-dynamic-price-pilot/`; this does not yet create a shared poster authoring or finance-wide conversion contract.
...
Deferred shop UI decisions are recorded rather than partially implemented. Jade Hearth needs one integrated pass beyond product cards: paper-banquet-menu rhythm, course order, vertical title slips, dishware ratios, and a clear `加菜 / Add dish` column should be solved together with its wider UI issues. Verdant Day keeps circular food photography but still needs ingredient-level salad customization: each ingredient must own explicit plus/minus controls and a visible price delta, which requires a modifier snapshot contract before UI work. Dash Grill's current card silhouette and placement must change; a receipt/order-ticket direction is only a candidate and needs a concrete visual prototype before implementation. None of these three facades changes in this Harbor slice.
