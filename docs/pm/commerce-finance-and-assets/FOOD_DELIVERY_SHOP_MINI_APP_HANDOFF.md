...
- The first menu-asset gate is delivered: all eleven `platform/menus/<merchant>/menu-item-01.png` paths now contain accepted `768x768` real-food representatives. `output/imagegen/baemin-platform/baemin-menu-pilot-contact-sheet-v1.png` records the cross-shop comparison; the fried-chicken pilot references the frozen original IP anchor. The remaining `menu-item-02..05.png` paths still use the diagnostic fallback until the next per-shop batches are accepted.
- The second Baemin asset gate is delivered: Camellia Beef Noodle House and South Wind Coconut Curry now use distinct local `1200x800` photographic covers; Berry Morning, Green Basket, Good Morning Bagel Coffee, and Elm Lane Dim Sum use original `768x768` true-alpha marks; all ten category shortcuts use the accepted 3D PNG family. The following V3 gate adds four `1360x640` merchant advertisements and connects about `96` to `111 KB` WebP runtime derivatives. The generation requests, PNG masters, contact-sheet/alpha evidence, and acceptance decisions live under `output/imagegen/baemin-platform/`. The current missing set is now `54`, plus the separately deferred delayed-order illustration.
...
- Peach Cloud now has a dedicated Vue app branch instead of conditional styling on the shared shop template. Its petal-pink, pink-mist, pale-green, and charcoal app frame, five Home category shortcuts, poster-led campaign system, and lightweight Pink Mist bottom navigation with Petal Rouge selected states deliberately contrast with Moon Bistro's dark side-rail tray menu. Home no longer duplicates the menu as Best Seller and Recommend product rails: it opens with one complete `2:3` vertical advertisement at a time, with campaign copy baked into the accepted poster image and no code-rendered mask, split copy panel, or adjacent-slide strip. Category selection and Search own menu discovery; Peach Club remains a separate action outside the poster. Discover/New reuses the full-poster carousel and then shows the original-size Golden Peach horizontal advertisement as one fixed `48 CNY` product. Clicking either a product poster or that horizontal advertisement opens the standard product detail sheet; the mascot poster opens Merch. There is no Pairing route and no Solo/Double/Party specification chooser. This palette is a later Peach Cloud brand decision layered over the Figma-derived information architecture; it does not change the Figma evidence or shared order runtime.
...
- The five-action footer uses Peach Cloud-local `shopView` route state. Its third action is `Discover`, with New, Club, and Merch sharing that selected state while retaining distinct URLs and back behavior. Empty Bag and Orders actions open their own empty-state pages; Bag owns mixed menu/merchandise quantity review and the existing confirmation-checkout entry; Orders opens a shop-scoped order list, and submitted orders open a dedicated detail/progress page. These surfaces reuse Food Delivery state and checkout actions without rendering the generic inline cart or support drawer beneath the Peach Cloud home.
...
- Peach Cloud's formal runtime pack now contains `33` assets under `public/images/ui-assets/apps/food-delivery/peach-cloud/`: `27` PNGs and `6` SVGs. It keeps one Club Hero PNG, three landscape campaign PNGs, three complete `1024x1536` poster PNGs, seventeen unique square product PNGs, three square merchandise PNGs, one original brand-mark SVG, and five Peach Cloud-prefixed category SVGs. The three new posters carry their own final advertising copy; the Golden Peach card keeps its product image on the right and renders the fixed `48 CNY` product identity separately on the left. The same peach/cloud brand mark appears in the Home Food Delivery folder and the shop bar. The five active category marks use one colorful, background-free IP illustration system, and the category container owns each pastel background. The original `vegan.svg`, `dessert.svg`, `drinks.svg`, `snacks.svg`, and `meal.svg` files remain unchanged as inactive reusable source marks for other shops. Image failures still show the shared high-contrast diagnostic placeholder and retain exact `data-required-asset` paths.
- The 17 product paths map one-to-one to 17 visually distinct `768x768` product photographs whose subjects match the current menu titles. The accepted mascot-market banner and plush, bag-charm, and tote product images were generated from the existing Hero as a high-fidelity identity reference. Exact prompts, accepted file copies, and acceptance records remain under `output/imagegen/peach-cloud-refresh/merchandise/` and `output/imagegen/peach-cloud-refresh/posters/`; older editable masters and the product contact sheet remain under `ads/` and `products/`. These masters are not runtime dependencies; the app displays only accepted copies under `public/`. The Peach Cloud pack remains separate from the Food Platform art pack.
...
River Noodles and Sugar Lane retain `20` pending PNGs, so no formal media acceptance may be claimed for either shop. Their street-route and display-shelf templates now degrade to template-native printed bowl/cake scenes instead of exposing the diagnostic broken-image artwork; the same identity carries into the shared item-detail sheet. Desktop Chromium and simulated Pixel 5 checks cover the fallback browse states, category switching, item visibility, and horizontal overflow. Their future candidate masters belong under `output/imagegen/river-noodles/` and `output/imagegen/sugar-lane/`; River Noodles must keep its river-teal stoneware/noodle language and Sugar Lane its berry/porcelain patisserie language rather than inheriting Daylight Cafe's bright morning terrazzo language.
...
Harbor Roast owns the non-general `harbor_roast_chain` facade, and its stable restaurant identity is `food_seed_harbor_roast`. It provides a drinks-first menu with twelve items across espresso classics, Harbor signatures, cold/blended drinks, and tea/counter bakes. Its route-driven Home uses three large clickable campaigns that open distinct member, seasonal-new-product, and Roast Passport pages; the Captain Roast commercial band and Passport now lead to a dedicated `supply` station, while each of four stable merchandise records opens a route-driven `supply-detail` advertisement page. The copper/blush/ink/line/cream palette, Captain Roast cup-and-anchor IP, focused Menu/detail, branded mixed bag/confirmation, shop order list, and order detail combine code-rendered controls and localized copy with the delivered Harbor media pack. The built-in restaurant automatically produces the `shop_app_food_seed_harbor_roast` App Store entry, so users can open the existing identity editor for display name, icon, cover, short description, tags, template selection, and Food Delivery folder placement without moving restaurant, menu, merchandise, reward, cart, or order ownership out of Food Delivery. User-provided Gallery icon/cover overrides remain higher priority than the delivered default App icon and cover. Fresh and existing saves receive missing stable IDs while same-ID user edits remain intact; explicit backup restore remains snapshot-faithful.
...
Harbor Roast's audited `34`-PNG asset contract is fully delivered under `public/images/ui-assets/apps/food-delivery/harbor-roast/`: one App icon, one transparent Captain Roast master, one `1200x750` App Store cover, three `1536x1280` Home carousel artworks, three `1200x1500` campaign posters, twelve `768x768` drink/bake product photos, one `1024x1536` Supply campaign advertisement, four `1024x1024` merchandise photos, six transparent order-status illustrations, and two transparent empty-state illustrations. Its exact palette is Copper `#C67C4E`, Blush `#EDD6C8`, Ink `#313131`, Line `#E3E3E3`, and Cream `#F9F2ED`; the older Petrol/Copper/Warm Ivory/Cranberry/Espresso photography capsule remains retired. Text, prices, buttons, navigation, stamp labels, and UI cards remain code-rendered. Focused Vitest and Playwright pass on desktop Chromium and simulated Mobile Chrome, including Home and Supply media decode, redemption eligibility, mixed cash/gift bag checkout, three campaign posters, dine-in routing and status art, App Store defaults, safe-area spacing, and page-level horizontal overflow. Named physical-device proof remains separate. Request logs, prompt files, source images, candidates, and acceptance evidence stay under `output/imagegen/harbor-roast/`; runtime must never read that directory.
