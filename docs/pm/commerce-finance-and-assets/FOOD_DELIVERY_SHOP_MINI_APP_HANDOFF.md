...
- Jade Hearth's order list receives the complete shop-scoped history, and Chat order links resolve directly to its dedicated `shopView=order&shopOrderId=...` detail. That page presents the Food Delivery-owned delivery timeline and confirm-delivery action, plus the existing explicit Wallet-record adapter without moving order or ledger ownership into the facade. It does not expose an update action that manufactures a delivery exception.
