...
- the exact Event Runtime log reference attached to an eligible canonical food-order event through the Food Delivery owner action
...
The clean-seed Home folder currently projects 15 Food Delivery-owned entries in stable order: Baemin, nine prior shops, and five reality-anchored Seoul restaurant Apps. Home owns only the `3 x 3` paged launcher shell and input containment. Food Delivery owns each restaurant/menu/cart/order record and stores only the stable Map `sourceId`; Map separately owns the bilingual place record, address, aliases, and provider-neutral coordinate. A real restaurant name or recognizable App mark is setting reference, not an affiliation claim. Public evidence may inform identity, cuisine category, branch, and address, but official menus, product images, prices, campaigns, advertisements, and combinations must not enter the Food Delivery seed catalog.

A Map `placeId` / Food Delivery `sourceId` association is context lineage, not a storefront route or authorization to dine in, pick up, add to a bag, place an order, or pay. That direct Map shortcut is `DEFERRED / EVENT_CONTRACT_REQUIRED`. A future place-session event may invoke an explicit Food Delivery-owned Adapter action only after separate acceptance; Event Runtime may coordinate eligibility, trigger, log, and projection, but Food Delivery continues to own restaurant, menu, bag, order, and consumption state, while Map continues to own place-session and location evidence.

EVE-4A's generic Food Delivery Event Surface is withdrawn at product review. A delivery exception belongs in Food Delivery's native fulfillment truth and existing Chat dispatch notification, not an added `Dispatch brief`; asking for an update must not manufacture a delay. Food Delivery retains exact one-to-one order/event/runtime-log provenance through its owner action. When a legitimate Runtime checkpoint triggers `rider_delay` or `eta_update`, Food Delivery updates canonical `order.etaMinutes`, appends its native order event, and pushes the existing service notification. No generic event browser, fake courier/contact choices, or second runtime is created.

Built-in independent-shop facades currently include Moon Bistro `food_seed_moon_bistro` / `dark_tray_menu`, Peach Cloud `food_seed_peach_cloud` / `dessert_window`, Dash Grill `food_seed_dash_grill` / `quick_service_chain`, Jade Hearth `food_seed_jade_hearth` / `jade_table_menu`, Verdant Day `food_seed_verdant_day` / `minimal_light_food`, Harbor Roast `food_seed_harbor_roast` / `harbor_roast_chain`, the reusable prepared-shop structures, and the five reality-anchored additions on `standard`. Their page composition and route-driven in-app views may differ by brand. Jade Hearth owns presentation for `shopView=menu|feast|bag|orders|order`; Verdant Day owns presentation for `shopView=menu|detail|bag|orders|order`. Those views remain facades over Food Delivery-owned restaurant, menu, cart, checkout, order, and event records rather than separate runtime owners. Normal hydration may add missing stable built-in IDs without overwriting same-ID saved records; explicit backup restore must remain snapshot-faithful and must not inject current seeds.
...
- Wallet owns card-appearance catalogs, ownership, unlock progress, and the selected appearance. An appearance binds to exactly one payment card; equipping a visual supplementary card must not create another account, balance, limit, or payment capability. A planned slot without accepted artwork remains non-equippable and must render as an intentional sealed state rather than a broken asset.
