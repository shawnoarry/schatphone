...
Current Food Platform asset note: `FOOD_DELIVERY_IMAGE2_ASSET_PROMPTS.md` freezes the platform's intended consumer name as `Baemin` and records a requirements-only multi-merchant photography handoff. Baemin-owned campaigns, categories, order states, and decorations may share the platform's mint-teal language; its eleven internal merchants may not. All merchant food must remain photographic, while their scene grammar ranges from pure-color editorial sets and real-food-plus-original-IP brand scenes to deliberately ordinary stall/mobile-camera photography. The `55` menu images are `11` separate five-image shop families, not one shared photography pack. The name remains code-rendered, and generated assets must not invent or approximate official Baemin logos or mascots. No new candidates, formal runtime copies, code rename, or visual acceptance is claimed by that requirement freeze.
