...
Current handoff: EVE-3 is complete. World Hub now exposes one Event Notebook read model over existing Event Instances, logs, Chat social proposals, and Map Journey proposals, with source/module/status filters, stable lineage review, stale-source fail-safe rows, and event-scoped notes. `store:simulation` V3 owns those durable notes, migrates V1/V2, includes them in backup/restore, and does not discard them when bounded runtime logs rotate. Notebook selection and note actions do not retrigger an event, mutate source records, create Reminders or Calendar plans, or inherit Cheats authority. Relationship classification gate audit, Chat social review, and WorldBook appBinding boundaries remain unchanged.
