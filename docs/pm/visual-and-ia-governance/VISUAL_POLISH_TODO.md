...
- **Slice B (12-marker 便签系统, user-approved model)** — `IN PROGRESS 2026-08-19 (lands, one known e2e follow-up)`: `src/lib/calendar-markers.js` holds the palette + 12 default markers (约会/生日/事业活动/纪念日/重要会议/其他/自定义1-6) + `normalizeCalendarMarkers`/`normalizeCalendarAppearance`/resolve helpers; events carry optional additive `markerId` (store normalize + updateEventDetails passthrough); `settings.appearance.calendar` normalizes on both hydration passes and rides the `system-settings` backup section; `CalendarEventEditor` has a 12-chip marker picker (clearable); `CalendarWorkspace` renders marker colors on month pills, agenda rows, and selected-day rows (border bar + label chip). Validated: lint, targeted tests 28/28 + new `tests/calendar-markers.test.js`, build. **Follow-up: the route-level marker e2e in `e2e/calendar-presentation.spec.js` was drafted but reverted uncommitted — it needs a write-settle-aware assertion (the marker write lands in the deferred mirror, so an immediate `localStorage` read can see stale state). Rebuild it around `expect.poll` on the persisted record.**
