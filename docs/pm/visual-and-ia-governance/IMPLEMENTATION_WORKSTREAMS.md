...
- style kits apply system theme + system App icon pack + optional recommended wallpaper without locking the combination; later changes are presented as a customized derivative and never mutate Home layout/widgets, Chat appearance, app skins, global custom CSS, commercial logos, or per-app overrides
- Appearance system-App icon themes operate on an explicit native-system allowlist such as Contacts, Settings, Widgets, Phone, and Gallery. Chat is an independent installed App outside this allowlist, alongside branded/commercial Apps. Icon themes are independent from light/dark themes, sit below explicit per-app overrides, and must never mutate independent App identities or in-page control glyphs such as Back, Search, and Close.
