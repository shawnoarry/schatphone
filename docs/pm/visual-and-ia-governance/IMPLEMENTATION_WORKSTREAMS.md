...
- `外观 / Appearance` as broader visual customization with four separate layers: day/night variant, system theme tokens/materials, curated native-system App icon pack, and optional one-click style-kit composition
- style kits apply system theme + system App icon pack + optional recommended wallpaper without locking the combination; a kit may also advertise an optional companion-widget collection, but the user must explicitly install it into Widget Center, repeated installation adds no duplicates, and no kit action mutates Home placement, templates, existing widgets, Chat appearance, app skins, global custom CSS, commercial logos, per-app overrides, or the user's system-App icon-priority preference
- Appearance system-App icon themes operate on an explicit native-system allowlist such as Contacts, Settings, Widgets, Phone, and Gallery. Chat is an independent installed App outside this allowlist, alongside branded/commercial Apps. Icon themes are independent from light/dark themes and sit below explicit per-app overrides by default. A separate persisted `prefer pack icons` choice may temporarily elevate the selected pack visual for allowlisted Apps only; it preserves saved display names and override data, restores the prior icon immediately when disabled, and must never mutate independent App identities or in-page control glyphs such as Back, Search, and Close.
- `应用商城 / App Store` as the standalone App Library-like entry-management host
- every real independent App Store entry supports a durable per-App display name and icon override; this includes Chat, Files, and Agenda Journey. Source-backed folder mini apps use their entry-presentation owner, while dynamic World Pack entries remain excluded until their identity-ownership contract is promoted.
