...
53. The App Store / pseudo-folder entry taxonomy is documented in `docs/product-decisions/APP_STORE_ENTRY_TYPES_AND_FOOD_SHOP_APPS.md`: App Store should manage independent apps, World Pack app entries, reusable shop entries, and system entries as user-visible launch entries while preserving backend ownership boundaries. `Shops` is now a storefront entry shape with limited binding targets, not a Food Delivery synonym.
...
56. App Store now has the V1/V2 entry-management taxonomy in code: filters split `Apps / World / Shops / System`, detail panels show runtime identity and ownership boundaries, standard apps can save phone-facing display names without changing backend ids/routes, and Food Delivery restaurants appear as shop entries that open with `restaurantId` context without becoming Home-placeable independent apps. Shop entries can also save App Store-side display/icon overrides, short descriptions, display tags, and shop-template selection; Food Delivery reads those presentation fields for launcher/store display while restaurant records, menus, carts, orders, Wallet handoffs, Map handoffs, and Chat service pushes remain source-owned. Product correction: the next App Store shop slice must generalize this to a binding-target model where Food Delivery is only the first allowed target and Shopping is the next allowed target.
...
7. Continue App Store from the landed entry-management taxonomy by first generalizing `Shops` into a reusable storefront binding model: add an explicit binding target field/UI, migrate current Food Delivery shops to `food_delivery`, add Shopping as the next allowed target, then continue cover editing, richer shop categorization/favorite/recent placement, create-shop flow, and pseudo-folder placement management without splitting source-module runtime ownership.
