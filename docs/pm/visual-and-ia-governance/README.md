...
- `音乐 / Music` is a standalone installed app with a restrained record-library identity rather than a system dashboard. Its primary spaces are `发现 / Discover`, `专辑 / Albums`, and `我的 / My Music`; Search is a top utility, while Queue and Music Settings remain utility actions instead of competing bottom tabs. Discover uses a full-width real-cover weekly recommendation, four explicit personal/catalog shortcuts, the album shelf, and a bounded real-history Recently Played overview; an empty history remains empty, and `See All` enters My Music -> Recently Played. Albums gets a catalog-specific stacked-cover header and complete grid, while My Music gets a personal cover mosaic, collection summary, read-only glance statistics, and one category navigation. Its Songs list contains only saved, favorite, or playlist-member tracks rather than the untouched demo catalog or playback history alone. Album card bodies open a cover-led Album Detail without auto-play, dedicated Play controls remain immediate, and track bodies consistently open Track Details while Play/Favorite/Playlist stay separate icon actions. Wider Album Detail retains the record-library sidebar; phone-sized detail becomes a focused drill-down that hides the three-space navigation and preserves the active in-app player. Playback, queue, Now Playing, lyrics, playlist intake, and Track Details sharing stay consumer-facing; the Now Playing favorite action changes immediately to a strong accent solid-heart `Added` state and feeds My Music Favorites. Music Settings separates `Add Music` (URL/local files) from external `Music Sources` (ChKSz, no-key Radio Browser live radio, and custom JSON), local audio remains Music-owned rather than entering Gallery, and the global floating player preserves the live session without covering Home or app-owned bottom controls. During an active Map journey, a separate focused Map panel offers quick music/radio choices above that floating layer; closing the Map panel leaves Music playback and the float intact. A Chat-returned track opens as details without auto-play.
...
- `电话 / Phone` now has a complete local call loop instead of a call-history-only MVP. Recents, Contacts, and Keypad are peer primary spaces; numbers and known contacts can start a simulated in-app call, Recents exposes separate detail and redial targets, and the active-call surface owns calling/connected state, duration, mute, speaker, keypad tones, and hang-up. Ending a connected call writes the existing Phone history and relationship fact, while ending before connection records a failed outgoing call without relationship impact. The manual record sheet remains a secondary history-repair tool, and the UI states clearly that no real carrier or VoIP connection is made.
...
- Visual work now uses zero or one specialist skill per round. Vague product wishes are valid intake: the assistant infers the product role, experience promise, page depth, direct content/media presentation, and justified expressive complexity, then defaults to the shortest product-complete implementation path. Product Design, multiple visual directions, or a disposable interaction prototype are optional escalation paths for consequential uncertainty rather than routine prerequisites. Playwright remains the single default verification path, with focused day/night, desktop/mobile, screenshot, overflow, page-error, and critical axe coverage for Home, Settings, and Appearance.
...
3. preserve Phone's focused local-call identity; keep Wallet, Stock, and Files on the shared shell baseline until their product roles justify bespoke visual systems;
