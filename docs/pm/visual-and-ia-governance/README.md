...
- `世界书 / WorldBook` now has a state-first Settings surface: active world overview, then a single-focus control deck for Setting Text, Current World Pack activation, profile templates, knowledge management, and advanced compatibility.
...
- WorldBook's control deck includes a `世界建立路径 / World setup path` rail: Setting Text, World Pack, Profile Templates, Knowledge, and Advanced compatibility are shown as a readable progression with state labels and direct panel switching.
