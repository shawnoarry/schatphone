...
5. roadmap 4.14 Event Experience, with EVE-2A's reusable contracts/fixtures and EVE-2B's registries, durable instances, local K-pop materialization, Simulation V2 migration/backup, and optional one-call text Composer complete; EVE-2C Map UI awaits separate acceptance.
...
| Map / Reminders / Phone | `Map partial; MJE-1 through MJE-4 integrated locally` | Map resolves one map per world and renders geographic packs through keyless OpenFreeMap + MapLibre, while fictional/custom packs and startup fallback remain local. Transport-aware planning, lifecycle/checkpoints, one non-blocking event family, Footprints, optional authored-place knowledge, and the concise ordinary place-focus overview/detail sheet are integrated. Explicit place sessions, `Enter`, dynamic event invitations, active exploration, large-map event cards, public-transit topology, and a standalone Transit app remain separately gated |
...
| Event Runtime / World Hub | `Partial / EVE-1 done` | safe foreground review and the pure Event Surface/empty-host-registry foundation are landed; Event receives no Home app and World Hub remains the hidden review/notebook entry. Location activation scopes and eligible-only place invitations are accepted documentation, while EVE-2 Map UI, Cheats, and background autonomy remain separate |
...
- cross-module Event Surface Projection and explicit host capability validation are implemented without Event UI or persistence; ordinary Map place-focus Stage 1 is integrated, while place-entry sessions, location-aware event families, dynamic invitations, coordinate/place event cards, World Hub Event Notebook, and Mini Scene expansion remain unimplemented and separately gated;
