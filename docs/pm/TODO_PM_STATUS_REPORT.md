...
| Event Runtime / World Hub | `PARTIAL_DONE` | EVE-4C owner-native commerce chain, Event Notebook, revision-bound Contacts identity projection, CJA-5 midpoint family, Event Policy V1 with a dedicated mobile-first Settings page, Event Instance cap removal, bilingual reason feedback, and optional retained Mini Scene history | identity remains read-only; organization ownership, new identity-conditioned event families, result rewrite/reroll controls, production Mini Scene trigger, and EVE-5 remain separately gated | `docs/pm/event-runtime-and-world-hub/STATUS_AND_HANDOFF.md` |
