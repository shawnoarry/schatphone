...
13. Users choose how many formal Home pages appear during normal use. Changing that count must only hide or reveal trailing pages; it must not change their templates, slots, or placed content.
...
- five retained editable formal Home pages, with two to five visible during normal use;
- persistent Dock.
...
The current release default shows three formal pages in normal mode. Users can choose two, three, four, or five visible formal pages from Appearance. Reducing the count hides only trailing pages; it does not remove their templates, slot placements, or content. Home edit mode always exposes all five formal pages so visual customization remains independent from the normal-mode count.
...
| Screen 3 | System Status, Quick Heart, Quick Disc | Personal system and expressive widgets |
| Dock | Chat, Contacts, Settings, Widgets | Globally reachable communication and device controls |
| App/Widget libraries | Network, Stock, Assets, duplicate formal-page App Store entry, and other unplaced entries | Available but not presented as equally release-ready defaults |
...
- the release default uses three visible formal pages; Appearance persists a user-selected normal-mode count from two through five;
- changing the visible count only hides or restores trailing retained pages. It never modifies their selected template, slot placement records, or placed content, and edit mode always exposes all five pages;
