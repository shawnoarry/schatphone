...
- 4.14 is `P1 PARTIAL_DONE / EVE-4A TECHNICAL_SPIKE_COMPLETE / PRODUCT_ACCEPTANCE_WITHDRAWN / EVE-4 RESET 2026-08-12`: Event is a cross-module causal chain, not a universal card system. EVE-2A/2B/2C and EVE-3 remain landed. The Food Delivery `rider_delay` spike keeps exact source/runtime lineage and now writes canonical Food Delivery ETA plus the native timeline/Chat notification, but its second-host `Dispatch brief`, expand/acknowledge UI, and manual query-that-manufactures-delay are withdrawn. The next EVE-4 slice must prove a real owner-native multi-node behavior chain; EVE-5 remains separately gated.
...
4. `CJA-3 TODO / USER_ACCEPTANCE_REQUIRED`: add an Agenda Journey V1 around one manual or Calendar-derived day plan, including current-origin departure recalculation and one linked Map Journey, without requiring random events.
5. `CJA-4 TODO / USER_ACCEPTANCE_REQUIRED`: add one Activity Session with explicit completion policy, minimize/navigation/reopen reconciliation, and a restrained Focus Companion baseline, with no claim of exact closed-app popup delivery.
...
Status: `P1 PARTIAL_DONE / EVE-0 DONE / EVE-1 DONE / EVE-2A DONE / EVE-2B DONE / EVE-2C DONE 2026-08-10 / EVE-3 DONE / EVE-4A TECHNICAL_SPIKE_COMPLETE / PRODUCT_ACCEPTANCE_WITHDRAWN / EVE-4B OWNER_NATIVE_FOOD_CHAIN_DONE 2026-08-14`
...
8. `EVE-4B OWNER_NATIVE_FOOD_CHAIN_DONE 2026-08-14`: implemented the first accepted non-card event chain across paid Food Delivery checkout, Wallet commerce payment, Map-owned courier journey/reroute, Food Delivery-native order messages, system notifications, and the Phone text-call shell. Runtime storage V4 persists only owner references and checkpoint lineage with deterministic random/no-event gates; no Food Delivery Event Surface, Event Home app, copied owner records, or `canonicalMutation` is introduced. Focused owner/runtime tests, full Vitest, lint, build, governance, and diff checks pass; direct Moon Bistro browser evidence passes on desktop Chromium and simulated Pixel 5. Full Playwright remains a bounded follow-up because the broad run did not complete within its time budget. EVE-5 remains separately gated.
9. `EVE-5 MINI_SCENE_AND_MEDIA_GATES_REQUIRED`: add richer text/interactive expansion only through the roadmap 4.8 shared Module after its persistence, Settings, security, and Presenter prerequisites are accepted; later CG remains separately gated from text materialization.
...
8. `P1 PARTIAL_DONE / EVE-4A PRODUCT_ACCEPTANCE_WITHDRAWN / EVE-4 RESET 2026-08-12`: 4.14 retains the pure Event Surface Projection, frozen EVE-2A contracts/fixtures, EVE-2B reusable runtime, the first Map/K-pop vertical slice, and the World Hub Event Notebook. Food Delivery retains exact-lineage owner integration and native ETA/timeline/Chat consequences but no Event Surface host. The next EVE-4 chain and EVE-5 Mini Scene/media expansion remain separately gated.
...
13. `P2 ARCHITECTURE_ACCEPTED / NOT_STARTED`: 4.12 Calendar/Agenda Journey orchestration is documented only. Dynamic current-origin departure, one linked Map Journey, validated arrival/entry, explicit activity completion policy, and Focus Companion ownership are accepted contracts, not implemented capability. CJA-1 Calendar information architecture requires a separate user acceptance decision; no Agenda Journey, Schedule Orchestrator, Activity Session, event popup, Narrative Timeline, or persistence implementation has started.
