...
3. user-test and harden the global world app-entry seam now that active World Pack app bindings appear in App Store/App Library/Home placement flows, Current World Pack hands off to the App Store `World` section instead of launching entries from Settings, and App Store explains the WorldBook-vs-App-Store ownership handoff before opening;
