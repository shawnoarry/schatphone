...
3. the current local integration passes lint, 210 Vitest files / 1497 tests, production build, focused proxy tests, and desktop/Pixel 5 restricted-relay checks; the broader Cloudflare build/governance/audit baseline remains proven from the prior round. Remote CI and deployed Pages include a direct user-configured provider model/connection/real-Chat/reload proof; Vercel and Cloudflare `ced45db` root/fail-closed proxy baselines are deployed, while the restricted dynamic relay is local and unpushed. Named physical-device plus independently rerunnable audit proof remain separate;
...
5. `P1 PARTIAL_DONE` 4.9/4.5 hosted release proof: GitHub Pages remote gates/base-path smoke and a direct user-configured provider model/connection/real-Chat/reload flow pass; the Git-connected Vercel root-path/fail-closed fixed proxy and Cloudflare Worker/static-assets `ced45db` baselines are deployed. A direct-default, explicitly selected, restricted per-request OpenAI-compatible relay for Vercel, Cloudflare, and GitHub Pages is implemented and locally validated but not pushed or deployed. Installed-PWA/relaunch, backup round trip, external protection checks, deployed relay smoke, and named true-device evidence remain.
