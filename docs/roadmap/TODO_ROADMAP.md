...
3. `P1 Core personal-life vertical slice` - `IN_PROGRESS`
   ...
   - `DONE 2026-07-31`: River Noodles, Daylight Cafe, and Sugar Lane each received nine-item menus, four shop-specific sections, existing-save missing-ID migration, and one-Hero/nine-product stable asset contracts. The 30 required PNGs remain ungenerated and unaccepted, so this records content and asset-list preparation only, not visual completion;
