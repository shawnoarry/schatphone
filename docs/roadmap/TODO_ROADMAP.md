...
4. `P1 Hosted product proof` - `IN_PROGRESS / AUTOMATED_HOSTED_BROWSER_PROOF_IMPLEMENTED / FOUR_SHARD_RELEASE_GATE_LOCAL_DONE 2026-08-31`
   ...
   - `AUTOMATED_BASELINE IMPLEMENTED 2026-08-31` at `38dc329`: GitHub builds emit `release.json` with the exact source commit, successful Pages runs trigger a separate Hosted Product Proof workflow, and an isolated Playwright configuration verifies the deployed `/schatphone/` base, resolved manifest ID/start URL/scope/icons, Service Worker control/cache, online-to-offline reload plus a second offline page, and a complete Settings backup exported from persisted owner data, restored into blank browser storage, and reopened with a completed recovery journal. Local static-preview acceptance passes 4/4 across desktop Chromium and simulated Pixel 5. Incognito is retained as an explicit automation-only installability boundary rather than relabeled OS installation; exact remote run evidence is recorded in `docs/qa/HOSTED_PRODUCT_PROOF_2026-08-31.md`;
   - `FOUR_SHARD_RELEASE_GATE LOCAL_DONE 2026-08-31`: PR CI and Pages run four independent one-worker, zero-retry Playwright shards with fail-closed JSON summaries and shard-specific diagnostics; Pages deployment waits for the exact-commit build plus the entire matrix. Local full-product acceptance is 422 passed / 4 accepted skips / 0 failed, and hosted static-preview proof remains 4/4. Exact remote CI/Pages/post-deploy evidence is still required after push;
   - `PAUSED_HANDOFF 2026-09-01`: runner-timezone and month-end fixture repairs plus bounded project-asset routing are committed locally at `4112875`, and confirmed image-bed KV quota evidence is committed at `7938e3a`; local exact-SHA hosted proof passes 4/4. The user stopped this stage before the quota reset, the scheduled continuation was deleted, `origin/main` remains `d26de0d`, and no replacement Pages or CI run has started. Resume only through the ordered probe -> focused asset E2E -> Pages -> automatic Hosted Product Proof -> manual CI sequence recorded in `docs/qa/HOSTED_PRODUCT_PROOF_2026-08-31.md`;
