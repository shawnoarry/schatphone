...
8. `CONTACTS-V3-2C PERSON_PROFILE_UX IN PROGRESS`: the person-first readable overview, direct persona/manual form entry, grouped confirmed fields, user-language second-level editor/review, category choice for proposed fields, and untouched Eva/Jackie representative fixtures are landed locally for joint product review. The latest refinement collapses fully empty reading categories into one pending-details summary, keeps partially filled category prompts, and focuses Persona classification by hiding the old reading list while explicitly preserving draft-only behavior. Existing Chat, relationship, memory, lifecycle, backup, and save behavior remains preserved; do not claim the full Contacts visual redesign complete before user acceptance;
