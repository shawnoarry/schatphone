...
5. `P1 PARTIAL_DONE` 4.9/4.5 hosted release proof: GitHub Pages remote gates/base-path smoke and the Git-connected Vercel root-path plus fail-closed AI-proxy baseline are deployed. A third Cloudflare Worker/static-assets root-path configuration passes local build, focused proxy/Worker tests, and Wrangler dry-run but still needs its first Git-connected deployment. Configured real-provider Chat, installed-PWA/relaunch, backup round trip, external protection checks, and named true-device evidence remain.
