...
3. `P1 Core personal-life vertical slice` - `IN_PROGRESS`
   ...
   - `DONE 2026-08-07`: the focused default Home release curation keeps normal use to two formal pages: glanceable widgets plus Wallet/Appearance/Photos/Camera, then Phone/Map/Calendar/Reminders/Shopping/Food Delivery. Network, Stock, Assets, System Status, Quick Heart, Quick Disc, and the duplicate formal-page App Store entry remain usable through App Store, Widget Center, Today View, or edit mode without appearing equally mature. Trailing empty pages stay hidden in normal mode and available in edit mode. Setup version 3 migrates only recognized earlier defaults while preserving customized layouts; focused unit and desktop/mobile Playwright cover migration, page count, route context, and App Store recovery.
...
4. `P1 DONE 2026-08-07`: the explicit custom role -> Chat journey, Wallet exact-money/card-pack foundations, role receiving-account V1, one ordinary Shopping life-consequence flow, focused Wallet quote explainability, and default Home release curation are complete at local desktop/simulated-mobile acceptance.
