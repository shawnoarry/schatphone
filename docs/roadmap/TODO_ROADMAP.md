...
5. `P1 PARTIAL_DONE` 4.9/4.5 hosted release proof: GitHub Pages remote gates/base-path smoke and a direct user-configured provider model/connection/real-Chat/reload flow pass; the Git-connected Vercel root-path plus fail-closed optional AI-proxy baseline and the Git-connected Cloudflare Worker/static-assets root path plus URL smoke are deployed. Installed-PWA/relaunch, backup round trip, external protection checks, named true-device evidence, and any future provider-specific optional-proxy proof remain.
