...
3. `P1 Core personal-life vertical slice` - `IN_PROGRESS`
   ...
   - `NEXT PRODUCT REVIEW`: curate the default Home release surface so partial utilities do not appear equally mature; hiding or demoting an entry changes discoverability and requires a focused product review.
...
4. `P1 IN_PROGRESS`: the explicit custom role -> Chat journey, Wallet exact-money/card-pack foundations, role receiving-account V1, one ordinary Shopping life-consequence flow, and focused Wallet quote explainability are done; default Home release curation now returns as the next product-choice review.
