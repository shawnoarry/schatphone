...
3. user-test and harden the global world app-entry seam now that active World Pack app bindings appear in App Store/App Library/Home placement flows, Current World Pack only tells users where to find them instead of launching or jumping from Settings, and App Store explains the WorldBook-vs-App-Store ownership handoff before opening; also preserve the distinction that World is an active-world source axis, while Shops is a storefront entry shape that can also be generated by a world pack when it has a supported binding target;
...
10. continue App Store shop-entry management from the V1/V2 taxonomy by first generalizing `Shops` beyond Food Delivery: add explicit binding targets, migrate current entries to `food_delivery`, add `shopping` as the next allowed target, and allow future world-pack-generated shops to live in `Shops` with `source: world_pack` plus a supported target. Display/icon/short-description/tag/template presentation is landed for Food Delivery-bound shop entries; covers, create-shop flow, richer shop grouping, and bound pseudo-folder placement can be added next, but catalog/menu/cart/order/runtime state must stay owned by the selected source module.
