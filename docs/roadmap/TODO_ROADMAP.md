...
Status: `P0 IN_PROGRESS / CMG-00, CMG-01, AND CMG-03 DONE 2026-08-20 / DCF-05 IN_PROGRESS`
...
5. committed relationship facts, Event Instances, Mini Scene artifacts, and other revisitable owner history cannot disappear because an array reached a row limit. Lists may page and AI prompts may select only relevant summaries;
...
| `DCF-05` | P0 | `IN_PROGRESS` | Replace the fixed 40 ms deferred-mirror wait with deterministic completion evidence. The persistence test must pass repeatedly and inside the full suite without increasing an arbitrary delay. | `CMG-00` | module architecture / controller fallback (`SKY-20250212UBG`) |
...
| `CMG-08` | P0 | `TODO` | Make Mini Scene request reuse deterministic and remove the 120-artifact silent truncation through an approved storage slice. The same request ID returns the committed artifact without another provider call; explicit regeneration uses a new revision/request. | `CMG-02` | module architecture + Event Runtime / PC-B |
...
16. added a separate place-media V1 registry and detail-sheet hero slot without changing the 106 canonical Seoul place records. The first governed pilot publishes seven verified licensed-photo derivatives, labels exact-place versus area-atmosphere truth, exposes author/source/license/change attribution, and gives fictional, player, missing, or failed media a non-evidentiary category fallback. Generated reconstruction remains schema-supported but unused in this pilot.
...
Status: `PARTIAL_DONE / CHKSZ_RADIO_BROWSER_LOCAL_AND_MAP_MEDIA_INTEGRATED_LOCAL`
...
23. `CHAT_DISCLOSURE_SUPPORTING_FACT_DONE 2026-08-18`: Chat now offers an explicit `让 TA 记住 / Remember for them` action only for one user-authored message in one role thread. The Chat Owner Adapter binds the exact conversation/message source and records one supporting-only `relationship_chat_user_disclosure` fact under `chat_disclosure__user_shared`, with no metric or stage change and source-level dedupe. This is a bounded continuity proof, not automatic message analysis, model-generated extraction, periodic consolidation, or a new memory store.
...
1. `P0 IN_PROGRESS / CMG-00, CMG-01, AND CMG-03 DONE 2026-08-20 / DCF-05 IN_PROGRESS`: the controller PC is the recorded fallback executor for `DCF-05`; no PC-B persistence work may run concurrently. `CMG-02` remains blocked until `DCF-05` completes, while `DCF-06` remains independently assignable only after non-overlapping paths are reserved.
...
5. `P2 PARTIAL_DONE / CHKSZ_RADIO_BROWSER_LOCAL_AND_MAP_MEDIA_INTEGRATED_LOCAL`: 4.13 Music has the installed app, browser playback, library/queue/search, generic JSON, no-key Radio Browser live radio, and ChKSz sources, direct HTTPS URL songs, Music-owned local-file import, Home/App Store integration, a global floating player, Chat track sharing, and an active-journey Map music/radio caller. Live-station uptime remains external; real-key rights/CORS smoke, true-device media behavior, optional local-binary backup/relink, Chat search, and external Map queue requests remain separately promoted work.
6. `P1 DONE 2026-08-10`: the explicit custom role -> Chat journey, Wallet exact-money/card-pack foundations, role receiving-account V1, one ordinary Shopping life-consequence flow, focused Wallet quote explainability, Wallet Activity search/monthly statements/verified payees, Wallet receipt sharing, configurable multi-screen Home release curation, the six-facade Shopping pseudo-folder expansion, and the 15-entry Food Delivery real-shop expansion with five Map-owned Seoul destinations are complete at local desktop/simulated-mobile acceptance.
7. `P1 PARTIAL_DONE 2026-08-09` 4.9/4.5 hosted release proof: GitHub Pages Run #31294272595, base-path smoke, and the prior direct-provider Chat/reload flow pass. Commit `a1418ed` is deployed through the Git-connected Vercel and Cloudflare builds; both restricted relay backends passed a no-secret upstream probe plus a GitHub Pages real-provider model-list and Chat smoke (`6` models, reply `OK`). Installed-PWA/relaunch, backup round trip, external protection checks, and named true-device evidence remain.
