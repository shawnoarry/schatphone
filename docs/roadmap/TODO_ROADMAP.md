...
- 4.14 is `P1 PARTIAL_DONE / EVE-3 DONE 2026-08-12`: Event remains a cross-module experience without a normal Home app. EVE-2A froze the reusable contracts and first K-pop archetype; EVE-2B added the durable reusable runtime; EVE-2C added Map V3 provenance/place sessions and the first Map/K-pop vertical slice; EVE-3 now adds the World Hub Event Notebook over existing runtime truth with event-scoped durable notes, filters, lineage review, and no event retrigger, Reminder/Calendar ownership, or Cheats authority. EVE-4 and EVE-5 remain separately gated.
...
2. `P0 Security/toolchain maintenance` - `PARTIAL_DONE`
   - `DONE 2026-08-12`: repository-owned artwork is externalized to the personal image bed without making it a shared user service. The 771-object migration plus a three-object project-brief batch are remotely verified and registered; authenticated batch publishing, centralized public runtime URLs, separate project/migration/Gallery credential boundaries, offline Git/CI gates, and copy-verify-remove home-PC archival are complete. Commits `f654b81` and `ffae433` are pushed; GitHub Pages Run `31537206567`, Vercel, and Cloudflare deployment checks passed; all three production hosts loaded 18/18 observed image-bed images without browser errors; and the temporary migration token is revoked while the upload/list-only project publisher remains active;
...
Status: `P1 PARTIAL_DONE / EVE-0 DONE / EVE-1 DONE / EVE-2A DONE / EVE-2B DONE / EVE-2C DONE 2026-08-10 / EVE-3 DONE 2026-08-12`
...
8. `P1 PARTIAL_DONE / EVE-3 DONE 2026-08-12`: 4.14 has landed the pure Event Surface Projection, frozen EVE-2A contracts/fixtures, EVE-2B reusable runtime, the first Map/K-pop vertical slice, and the World Hub Event Notebook with unified runtime review plus durable event-scoped notes. EVE-4 additional hosts/content and EVE-5 Mini Scene/media expansion remain separately gated.
