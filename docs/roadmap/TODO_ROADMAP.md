...
3. `P1 Core personal-life vertical slice` - `IN_PROGRESS`
   ...
   - `DONE 2026-08-09`: the user-approved Baemin completion batch connects the remaining nine five-image merchant families and seven photography-merchant order marks without changing Food Delivery behavior. Thirty-six menu candidates are promoted through an explicit selection manifest and deterministic `768x768` RGB exports; the V2 black-garlic boneless chicken is selected over rejected V3/V4 variants, the mistaken fries retouch remains excluded, and the corrected half-and-half wings resolve to Hwadeok Pizza. All eleven merchant directories now expose `menu-item-01..05`, while all eleven compact order identities resolve through four existing logo marks plus seven new true-alpha RGBA marks. Accepted and runtime hashes match. Platform decoration/order-flow placeholders, existing-cover re-review, the deferred delayed-state illustration, and named physical-device proof remain separate;
