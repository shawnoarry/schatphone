...
- The Sources task panel now reads as a source-control console with active/review/available/disabled stats, system fallback status, linked-source cards, and separated destructive removal actions. Its source-link/picker/diff display read-model is now extracted into `src/composables/useWorldBookSourceModel.js`, so future governance should move to Current World Pack review/display instead of repeating this source seam.
- WorldBook's Current World Pack, Knowledge, and Templates task panels now have their first internal craft pass; knowledge creation/editing is a focused bottom sheet, while the Knowledge panel's usage/readiness counts, filters, sorting, deep-link scoping, and role/Chat binding display now sit behind `src/composables/useWorldBookKnowledgeModel.js`, and the Templates panel's stats, universal/current-world rows, Contacts handoff copy, and state/toggle labels now sit behind `src/composables/useWorldBookProfileTemplateModel.js`.
