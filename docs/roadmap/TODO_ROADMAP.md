...
8. `CONTACTS-V3-2C PERSON_PROFILE_UX IN PROGRESS`: the person-first readable overview, grouped confirmed fields, user-language second-level editor/review, category choice for proposed fields, and untouched Eva/Jackie representative fixtures are landed locally for joint product review. The latest refinement establishes one overview-level `Import persona` entry plus the secondary manual form, removes the duplicate second-level import action, and accepts pasted text or local TXT/Markdown/JSON as review-only source. Invalid, empty, oversized, unsupported, or unreadable files preserve the prior source and canonical profile. Empty-category summaries and Persona focus behavior remain intact. Existing Chat, relationship, memory, lifecycle, backup, and save behavior remains preserved; do not claim the full Contacts visual redesign complete before user acceptance;
