...
| Phone / 电话 | Local role-call log MVP with recent calls, missed/completed counters, manual records, persistence, backup/restore, and regression tests. | Not yet connected to Chat roles, shell notifications, Calendar cues, or AI call summaries. | Data lives in `src/stores/phone.js`; current page records local simulated calls only and does not dial real phone calls. | Connect missed-call records to notification or Calendar cues before expanding AI call summaries. | P2 |
...
| Wallet / 钱包 | Local virtual ledger MVP with balance summary, manual transfer records, delete flow, persistence, and regression tests. | Not yet connected to Chat transfer cards or any economy/consumption loop. | Data lives in `src/stores/wallet.js`; current page records manual virtual transfers only. | Next step is attaching Chat `transfer_virtual` blocks as ledger sources, not real payment integration. | P2 |
