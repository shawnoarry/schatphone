...
| WorldBook / 世界书 | Global worldview + knowledge points are split and bindable into Chat; usage visibility, filters/sorting, keyword search, tag filtering, in-place edit flow, Calendar-side related-knowledge visibility, and Chat-side injection visibility are now available. | Users can now manage knowledge points well, and Chat/Calendar both expose active consumption, but Map and deeper deep-link workflows are still thin. | Consumption is now visible in Chat and Calendar with prompt-level verification, but broader module usage is still early. | Extend the same read-only relevance strip into Map or add direct WorldBook deep-link filtering from Calendar or Chat. | P1 |
...
23. Next best slice: extend WorldBook read-only relevance into Map or add Calendar/Chat deep-link filtering into WorldBook.
    下一步最推荐：把 WorldBook 的只读关联扩到 Map，或补 Calendar / Chat 直达 WorldBook 的深链筛选。
