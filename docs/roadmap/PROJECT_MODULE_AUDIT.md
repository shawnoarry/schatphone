...
| Files / 文件 | Internal metadata-only storage/index component with search, favorite, delete, quick note creation, local metadata import, backup/diagnostics coverage, and hidden standalone frontend entry. | It should not appear as a generic file-manager app; user-facing files/assets belong in their owning modules such as Gallery, Contacts, Chat, Map, Calendar, and Settings. | Data lives in `src/stores/files.js`; original file content is not copied, uploaded, or stored. Home/More entries are hidden while `/files` remains available for compatibility/internal use. | Only expand Files when another module needs an internal metadata bridge; keep user-facing management in the owning module. | P2 |
| More / 更多 | MVP quick entries plus persisted experimental toggles that now drive Home smart panel, Lock focus-mode condensation, and More scene-switch preview. | Toggles have first visible consumers, but remain lightweight Labs affordances rather than full automation. | Feature flags live under `settings.more.featureToggles`; current consumers are intentionally low-risk UI gates. | Do not add more toggles until at least one existing toggle graduates from preview to real workflow ownership. | P2 |
...
| Phone / 电话 | Local role-call log MVP with recent calls, missed/completed counters, manual records, shared missed-call notifications, Calendar callback cue handoff, persistence, backup/restore, and regression tests. | Not yet connected to Chat roles or AI call summaries. | Data lives in `src/stores/phone.js`; current page records local simulated calls only and does not dial real phone calls. New missed-call records create shell notifications and Calendar cues, but do not schedule real push directly. | Defer AI call summaries until the Calendar cue loop is product-clear. | P2 |
| Calendar / 日历 | Lightweight reminder surface consumes Map-derived area feedback cues and Phone missed-call callback cues, lets users confirm/pin/dismiss supported cues, stores confirmed reminders as Calendar events, supports event time editing, schedules real push from confirmed event times, and shows push readiness/status/history notes. | Calendar has local schedule logs, but still lacks server-side delivery receipts and a fuller event list. | Calendar reads Map cues, owns Phone callback cues, writes user choice back where appropriate, owns a persisted event store, preserves user-edited event times, and tracks scheduled push state/history. | Add server-side delivery receipts if the push relay exposes them, or build a fuller event management surface. | P1 |
| Wallet / 钱包 | Local virtual ledger MVP with balance summary, manual transfer records, Chat transfer-card ledger sync, source counts/filters/badges, delete flow, persistence, and regression tests. | Chat transfer cards now create visible virtual ledger entries, but there is still no broader economy/consumption loop. | Data lives in `src/stores/wallet.js`; current page records manual virtual transfers, and Chat `transfer_virtual` messages are stored as deduped `chat_transfer` source entries with UI source visibility. | Defer economy simulation until more narrative consumers exist; do not treat this as real payment integration. | P2 |
