...
const participantName = computed(() => {
  ...
  if (props.order?.fulfillmentMode === 'pickup') {
    return name || props.order?.restaurantName || t('小店服务', 'Shop support')
  }
...
const canRequestAddressChange = computed(() => Boolean(
  props.order && props.order.fulfillmentMode !== 'pickup' && selectedAnchor.value && !['delivered', 'cancelled'].includes(props.order.status) &&
...
<template>
  <section v-if='order' class='food-order-chain' data-testid='food-delivery-order-chain'>
    ...
    <div v-if='expanded' id='food-delivery-order-chain-body' class='food-order-chain__body'>
      ...
      <div class='food-order-chain__payment' data-testid='food-delivery-order-payment'>
        <div>
          <p class='food-order-chain__eyebrow'>{{ t('付款', 'Payment') }}</p>
          <strong>{{ order.paymentStatus === 'completed' ? t('Wallet 已付款', 'Paid with Wallet') : t('未记录付款', 'Payment not recorded') }}</strong>
          <small v-if='order.paymentRef?.receiptNumber'>{{ order.paymentRef.receiptNumber }}</small>
        </div>
        <button v-if='order.paymentRef?.transactionId' type='button' class='food-order-chain__secondary' data-testid='food-delivery-order-open-payment' @click='emit("open-payment")'>{{ t('查看付款记录', 'View payment') }}</button>
      </div>
    </div>
  </section>
</template>
...
.food-order-chain__header,.food-order-chain__status,.food-order-chain__conversation-head,.food-order-chain__actions,.food-order-chain__payment { display:flex; align-items:center; justify-content:space-between; gap:.7rem; }
...
.food-order-chain__payment { margin-top:.8rem; padding:.8rem; border-radius:1rem; background:rgba(255,255,255,.05); }.food-order-chain__payment strong,.food-order-chain__payment small { display:block; }.food-order-chain__payment strong { margin-top:.2rem; font-size:.78rem; }.food-order-chain__payment small { margin-top:.2rem; color:#9ba9ba; font-size:.62rem; }
</style>
