...
<header>
<button class="back" type="button" :aria-label="tx('返回','Back')" data-testid="parcel-back" @click="pushReturnTarget(router,route,'/home')"><i class="fas fa-chevron-left"/></button>
<div class="logo"><i class="fas fa-box"/></div>
<div class="brand"><b>{{tx('递送','POSTA')}}</b><small>{{tx('包裹与寄件服务','Parcel & sending service')}}</small></div>
<span class="pinned-count">{{shell.pinnedShipmentIds.value.length}}</span>
</header>
<section v-if="shell.activeTab.value==='track'" class="page" data-testid="parcel-track">
<div class="hero">
<div class="hero-head"><small>{{tx('包裹板','PARCEL BOARD')}}</small><i class="fas fa-arrow-down-long"/></div>
<h1>{{tx('每一件东西，\n都有来路。','Every parcel\nhas a trail.')}}</h1>
<i class="barcode" aria-hidden="true"/>
</div>
<label class="search"><i class="fas fa-magnifying-glass"/><input v-model="query" data-testid="parcel-search" :placeholder="tx('运单号、物品或寄件方','Tracking code, item, or sender')"/></label>
<div class="shipments">
<button v-for="item in results" :key="item.id" type="button" :data-testid="'parcel-shipment-' + item.id" @click="selected=item">
<div class="status" :class="item.status"><i :class="statusIcon(item.status)"/><span>{{lx(item,'status')}}</span></div>
...
<div class="type-row">
<button type="button" :class="{active:shell.sendDraft.value.itemType==='parcel'}" @click="shell.updateSendDraft({itemType:'parcel'})"><i class="fas fa-box"/>{{tx('包裹','Parcel')}}</button>
<button type="button" :class="{active:shell.sendDraft.value.itemType==='document'}" @click="shell.updateSendDraft({itemType:'document'})"><i class="fas fa-envelope"/>{{tx('文件','Document')}}</button>
</div>
...
<div class="inbox">
<article><i class="fas fa-box-open"/><div><b>{{tx('自提件已入柜','Pickup parcel is ready')}}</b><p>{{tx('录音棚线材包已到圣水自提柜。','The studio cable kit is at the Seongsu locker.')}}</p></div><span>16:20</span></article>
<article><i class="fas fa-triangle-exclamation"/><div><b>{{tx('一条来源已失效','One source has expired')}}</b><p>{{tx('海外样品包需要未来物流 owner 刷新。','The overseas sample needs a future logistics owner refresh.')}}</p></div><span>09:05</span></article>
</div>
</section>
...
<aside v-if="selected" data-testid="parcel-detail">
<button class="close" type="button" :aria-label="tx('关闭','Close')" @click="selected=null"><i class="fas fa-xmark"/></button>
...
<div class="route-line"><i/><i/><i/></div>
<h2>{{lx(selected,'status')}}</h2>
<p>{{lx(selected,'last')}}</p>
<b class="eta-line">{{lx(selected,'eta')}}</b>
<div v-if="selected.mapPlaceId" class="map-ref"><i class="fas fa-map-location-dot"/><span>{{tx('引用现有 Map 地点','References existing Map place')}}</span><small>{{selected.mapPlaceId}} · READ ONLY</small></div>
<button class="pin" type="button" :aria-pressed="shell.pinnedShipmentIds.value.includes(selected.id)" @click="shell.togglePinned(selected.id)"><i class="fas fa-thumbtack"/>{{shell.pinnedShipmentIds.value.includes(selected.id)?tx('取消置顶','Unpin'):tx('置顶包裹','Pin parcel')}}</button>
...
<style scoped>
.posta{--bg:#f3eee2;--paper:#fffdf7;--ink:#21251f;--muted:#6e6a5e;--line:#d9d2c0;--red:#c03a2b;--red-deep:#9c2d21;--stamp:#3d5a6c;--green:#4e795b;--amber:#a8722a;--stale:#8a8578;position:relative;height:100%;overflow:hidden;color:var(--ink);background:var(--bg);font-family:"Avenir Next","Noto Sans CJK SC",sans-serif}
...
.brand b,.brand small{display:block}.brand b{letter-spacing:.04em}.brand small{color:var(--muted);font-size:9px;font-weight:700;letter-spacing:.12em}
...
.page{height:calc(100% - 86px);padding:25px 28px 110px;box-sizing:border-box;overflow-y:auto}
...
.search input{min-width:0;flex:1;border:0;outline:0;color:inherit;background:transparent;font-size:14px}
...
.status span{font-size:9px;font-weight:900;letter-spacing:.08em}
...
.title{max-width:680px;margin:18px 0 25px}
.title h1{margin:8px 0;font-size:34px;font-weight:900;line-height:1.05}
.title p{color:var(--muted);line-height:1.6}
.send-form{max-width:720px;padding:24px;display:grid;gap:17px;border:2px solid var(--ink);border-radius:6px;background:var(--paper)}
...
.send-form input,.send-form textarea{width:100%;box-sizing:border-box;border:1.5px solid var(--line);border-radius:6px;color:inherit;background:var(--bg);font-size:14px}
...
nav span{margin-top:4px;font-size:9px;font-weight:700;letter-spacing:.05em}
...
aside::before{content:"";position:absolute;top:0;left:0;right:0;height:14px;background:repeating-linear-gradient(90deg,var(--red) 0 22px,var(--paper) 22px 30px,var(--stamp) 30px 52px,var(--paper) 52px 60px)}
aside>.close{position:absolute;right:18px;top:calc(30px + env(safe-area-inset-top))}
...
.route-line{height:35px;margin:18px 0 4px;display:flex;align-items:center}
.route-line i{width:14px;height:14px;border-radius:50%;border:3px solid var(--red);background:var(--paper)}
.route-line i:last-child{background:var(--red)}
.route-line i:nth-child(2){height:0;flex:1;border:0;border-top:3px dashed var(--red);border-radius:0;background:transparent}
