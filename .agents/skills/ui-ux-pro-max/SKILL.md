---
name: ui-ux-pro-max
description: "UI/UX evidence baseline for SchatPhone visual work. Use near the start of every UI beautification, redesign, new page, brand, motion, 3D/spatial, or visual-review task to query the project-local database for product patterns, styles, palettes, typography, accessibility, animation, charts, icons, Vue, or Three.js guidance. Skip pure backend and non-visual work."
---
...
This reviewed copy is vendored inside the SchatPhone repository. Run it from the repository root with the project-relative path below. If the current working directory is elsewhere, resolve the repository root first instead of relying on a machine-specific absolute path or `CLAUDE_PLUGIN_ROOT`:
