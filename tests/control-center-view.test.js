...
describe('ControlCenterView', () => {
  ...
  test('renders simulation runtime state as a read-only World Hub panel', async () => {
    ...
    expect(wrapper.get('[data-testid="control-center-world-panel"]').text()).toContain('World usage evidence')
    expect(wrapper.get('[data-testid="control-center-world-panel"]').text()).toContain('Advanced review')
    expect(wrapper.find('[data-testid="world-understanding-run-ai"]').exists()).toBe(false)
    expect(wrapper.get('[data-testid="world-understanding-runtime-evidence"]').text()).toContain('world_context_sci_fi')
