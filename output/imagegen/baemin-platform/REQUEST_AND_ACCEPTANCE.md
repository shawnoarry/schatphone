...
- Menu runtime: every `platform/menus/<merchant>/menu-item-01.png` path resolves to a local `768x768` PNG. Reverse Station Hanwoo Gukbap and Sushi Hana now additionally resolve accepted `menu-item-02.png` through `menu-item-05.png`; the nine other merchants retain their pending `02..05` diagnostic fallbacks.
...
- Remaining count: V2 reduced the current Baemin missing set from `74` to `58`; the four V3 advertising covers reduced it to `54`; Hanwoo menu batch V1 reduced it to `50`; Sushi Hana menu batch V1 reduces it to `46`, plus the separately deferred future delayed-order illustration.
