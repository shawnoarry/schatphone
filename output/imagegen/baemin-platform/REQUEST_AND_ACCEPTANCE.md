...
- complete Reverse Station Hanwoo Gukbap's five-image menu family by generating, accepting, and connecting `menu-item-02.png` through `menu-item-05.png` without replacing its accepted pilot.
- complete Sushi Hana's five-image menu family by generating, accepting, and connecting `menu-item-02.png` through `menu-item-05.png` without replacing its accepted pilot.
...
- Hanwoo menu batch V1 request file: `requests-hanwoo-gukbap-batch-v1.jsonl` contains four distinct `gpt-image-2` / `high` jobs for menu items `02..05`. Untouched `1254x1254` provider candidates remain under `candidates/menu-batches/hanwoo-gukbap/`; deterministic Pillow LANCZOS exports under `accepted/menu-batches/hanwoo-gukbap/` drive exact `768x768` runtime files.
- Sushi Hana menu batch V1 request file: `requests-sushi-hana-batch-v1.jsonl` contains four distinct `gpt-image-2` / `high` jobs for menu items `02..05`; request SHA-256 is `8138f2b20767a54b27c81e113eca7619c966901e83aaec3763257d7c9f2f0378`. `requests-sushi-hana-batch-v1-retry.jsonl` contains the single item-04 count correction; request SHA-256 is `1df161d36a1d2cbe898dc1c658d193107b37b2b49605021793167b14a60f8903`. Untouched provider candidates remain under `candidates/menu-batches/sushi-hana/`; deterministic Pillow LANCZOS exports under `accepted/menu-batches/sushi-hana/` drive exact `768x768` runtime files.
- Remaining menu requests are recorded in `requests-remaining-menu-batches-v1-modern.jsonl`, `requests-salad-day-batch-v1-modern.jsonl`, and `requests-hwadeok-pizza-batch-v2-modern.jsonl`. Targeted texture requests are retained in `requests-texture-fixes-v1.jsonl` and `requests-texture-regenerations-v2.jsonl`; the rejected V4 black-garlic recognition attempt remains in `requests-black-garlic-recognition-v3.jsonl` as evidence only.
- `requests-platform-merchant-marks-v1-modern.jsonl` records the seven photography-merchant identity requests. True-alpha candidates under `candidates/merchant-marks/v1-modern-alpha/` drive accepted and runtime files without resizing.
- `accepted/integration-selection-v1.json` is the authoritative selection manifest for this integration. It records all 36 menu source candidates, seven merchant marks, the user-selected V2 black-garlic chicken, and the rejected fries/black-garlic alternatives.
...
- Menu runtime: all `55` paths under `platform/menus/<merchant>/menu-item-01.png` through `menu-item-05.png` resolve to local `768x768` RGB PNGs. Each of the eleven merchant directories contains exactly five decodable files.
- Hanwoo menu batch V1: the accepted contact sheet `baemin-hanwoo-gukbap-menu-batch-v1-contact-sheet.png` confirms one coherent neighborhood soup-shop family with five distinct silhouettes. The new assets preserve the worn wood, modest ceramic/earthenware, mixed window/interior light, and documentary texture of item `01`, while keeping the two-person kimchi beef hot pot, light beef-bone breakfast set, spicy shredded-beef hangover soup, and seafood kimchi pancake semantically separate. No candidate contains readable text, price, Logo, people, real-brand packaging, or watermark.
- Hanwoo export gate: all four candidates are opaque `1254x1254` RGB PNGs. Accepted/runtime exports are exact `768x768` RGB PNGs, and accepted/runtime SHA-256 matches for every file. Accepted hashes are `44840e10418ed574929f23d8963230d1eec3430d281971d62d15d20cd55c24e3`, `f22b0c54d1b30d39021e75b42fef4bdaeb50a4bc586f7065ef0c04e4e9c9f60b`, `0a569eb972380342e7394d0011eb13d692d3be6859aaa5e4948ec67e3e26d7e8`, and `00d5ed17ff46d32a68782d70e8df93c1ea122082e997d61e14857f1a8e2cfacc` for items `02..05` respectively.
- Hanwoo page gate: focused Playwright passes in desktop Chromium and simulated Pixel 5. All five merchant-page images decode from their exact stable paths at `768x768` without `data-asset-missing`; top/end screenshots confirm readable card crops, stable add actions, and no horizontal overflow. The retained page review is `baemin-hanwoo-gukbap-playwright-review.png`. This is not named physical-device proof.
- Sushi Hana menu batch V1: the accepted contact sheet `baemin-sushi-hana-menu-batch-v1-contact-sheet.png` confirms one coherent cool Japanese editorial family with five distinct silhouettes. The new assets preserve the accepted pilot's pale celadon and blue-grey ceramics, light bamboo, restrained textile accents, soft bright natural light, and generous clean space while separating the rectangular pork-katsu bento, round salmon-and-roe chirashi, long-plate eel-avocado roll, and deep-bowl red-miso clam soup. No candidate contains readable text, price, Logo, people, real-brand packaging, or watermark.
- Sushi Hana retry decision: first-round items `02`, `03`, and `05` were accepted. First-round item `04` remains as rejected evidence because it returned seven roll pieces despite the exact eight-piece request. `menu-item-04-v2.png` corrects the count to eight complete visible pieces while preserving the shop capsule and is the only candidate promoted to runtime item `04`.
- Sushi Hana export gate: the five generated candidates are opaque `1254x1254` RGB PNGs. Accepted/runtime exports are exact `768x768` RGB PNGs, and accepted/runtime SHA-256 matches for every promoted file. Accepted hashes are `6f1b2c3cd28e653d1001f889a6944a088fa15c144f2626d2b06d5ebe982abc2f`, `3f7517d4d7db8102e179794b99823648da3829fa1fe36c41ff6c9c263d646a9d`, `199b13cd1cc5eb5f5d3377ab8e913d26964947db2b1ff4ab282c876bab73d4da`, and `237908c8df80cb570395d1ca870fe21552bd2737799ae78a3fcc172fe411f97d` for items `02..05` respectively.
- Sushi Hana page gate: focused Playwright passes in desktop Chromium and simulated Pixel 5. All five merchant-page images decode from their exact stable paths at `768x768` without `data-asset-missing`; top/end screenshots confirm readable card crops, stable add actions, and no horizontal overflow. The retained page review is `baemin-sushi-hana-playwright-review.png`. This is not named physical-device proof.
...
- Merchant marks: Berry Morning, Green Basket, Good Morning Bagel Coffee, and Elm Lane Dim Sum retain their distinct true-alpha logo marks. Hanwoo, Sushi Hana, Hwadeok Pizza, Salad Day, Crispy Chicken House, Camellia Noodles, and Coconut Curry now add seven dedicated order/status marks under `platform/orders/merchant-marks/`. All eleven compact identities are `768x768` RGBA PNGs with real transparent pixels and no official brand approximation.
...
public/images/ui-assets/apps/food-delivery/platform/menus/*/menu-item-01.png ... menu-item-05.png
...
public/images/ui-assets/apps/food-delivery/platform/orders/merchant-marks/platform-merchant-mark-*-01.png
```
