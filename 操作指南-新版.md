...
本指南用于日常开发、验收和提交流程，内容已同步到当前代码主线。
...
- 顶部结构：返回桌面 + 用户状态 + 新建 + 添加服务号
- 用户状态支持：空闲 / 忙碌 / 离开（含状态灯）
- 可进入“会话通讯录”管理对话对象

### 6.2 会话通讯录（`/chat-contacts`）

- 分层管理：角色/群聊 与 服务号/公众号
- 支持：新建、编辑、删除、直接进入会话
- 与全局 Contacts 分离，专用于聊天模块
...
错误提示说明：
- 当前已支持错误分级提示：URL、鉴权、404、限流、超时、网络/CORS、服务端异常。

## 9. 数据与隐私规则

- 当前版本默认本地持久化（浏览器存储），不做平台云端托管。
- 仅在用户触发时将上下文发送到用户配置的 API 地址。
- 删除会话对象是删除本地数据。
...
6. 涉及输入补保存按钮或明确自动保存提示
7. 更新文档：`PROJECT_STATUS.md`、`ARCHITECTURE.md`、`CHAT_PROGRESS.md`
