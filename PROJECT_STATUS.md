...
The project is in "stable core path + immersive-phase transition" stage: baseline shell is stable, and work focus is now moving to real-time world rhythm and state continuity.  
项目处于“主链路稳定 + 沉浸阶段过渡”状态：壳层基线已稳定，当前工作重心转向现实时间节奏与状态连续性。
...
5. System-owned truth minimal layer (`P0-B1`) / 系统真值最小层（`P0-B1`）  
Start relationship/event/time truth fields to keep cross-provider continuity stable.
补齐关系/事件/时间真值字段，保证跨供应商连续性稳定。

### P1

1. Structured block policy hardening (`P1-2`) / 结构化消息策略加固（`P1-2`）
2. Settings UX refinement (`P1-3`) / 设置体验优化（`P1-3`）
