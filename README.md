...
- AI single-message semantic revision is the current top priority (`P0-1` in `PRODUCT_MANAGER_PRIORITY_PLAN.md`)  
AI 单条语义修订是当前最高优先级（见 `PRODUCT_MANAGER_PRIORITY_PLAN.md` 的 `P0-1`）
- Chat top-level bottom feature dock is pending (`P0-2`)  
Chat 一级页底部功能 Dock 待落地（`P0-2`）
- User rich-message lane still needs hardening/polish for final acceptance (`P0-C2` / PM `P0-4`)  
用户富消息链路仍需加固与体验打磨后再收口（`P0-C2` / 产品经理优先级 `P0-4`）
- Gallery global asset center and role-bound reuse are pending (`P0-5` / `P0-6`)  
相册全局素材中心与角色绑定复用待落地（`P0-5` / `P0-6`）
- Asset binary persistence/export hardening remains pending for long-term storage growth (`P0-7`)  
素材二进制持久化/导出加固仍待完成（`P0-7`，面向长期存储增长）
