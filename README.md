...
Start here / 文档总入口：
- `docs/README.md`: documentation map by function, current reading order, and archive rules / 按职能分类的文档地图、当前阅读顺序与归档规则

Current authority / 当前权威来源：
- `docs/roadmap/TODO_ROADMAP.md`: only live execution board / 唯一动态执行看板
- `docs/roadmap/PROJECT_MODULE_AUDIT.md`: candidate task pool, not a second roadmap / 候选任务池，不是第二路线图
- `docs/process/AI_WORK_MODE.md`: workflow and documentation governance / 流程规范与文档治理

Main folders / 主要目录：
- `docs/pm/`: PM weekly status and product overview / 产品经理周报与产品总览
- `docs/roadmap/`: roadmap and module task pool / 路线图与模块任务池
- `docs/overview/`: project master guide / 项目总说明
- `docs/process/`: workflow, operation guide, validation flow / 流程、操作指南、验收流
- `docs/architecture/`: architecture and contracts / 架构与契约
- `docs/product-decisions/`: current topic-level product decisions / 当前专题产品决策
- `docs/strategy/`: long-range product and technical strategy / 长线产品与技术策略
- `docs/templates/`: reusable PM and AI collaboration templates / 可复用 PM 与 AI 协作模板
